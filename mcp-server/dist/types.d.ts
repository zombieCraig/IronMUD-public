...
/** True when the quest is active AND every objective is satisfied. */
 | {
    kind: "quest_completable";
    vnum: string;
}
/** Vampires only; mortals fail unconditionally. */
 | {
    kind: "humanity_at_least";
    threshold: number;
}
/**
...
export type ItemTriggerType = "on_get" | "on_drop" | "on_use" | "on_examine" | "on_look" | "on_prompt" | "get" | "drop" | "use" | "examine" | "look" | "prompt";
...
/** Kill any of the listed prototype vnums until the shared counter hits `count`. */
 | {
    ...
    vnum: string;
    qty: number;
    return_to_mob_vnum?: string | null;
} | {
    kind: "visit_room";
    vnum: string;
} | {
...
export type AchievementCategory = "skill" | "combat" | "crafting" | "exploration" | "social" | "wealth" | "builder";
