    ...
    },
    {
      ...
      properties: {
        ...
        delta: {
          type: "number",
          description:
            "Shift the player's morality slider. Positive pushes toward Good, negative toward Evil; clamped to [-200, 200]. Crossing a tier announces itself, sub-tier nudges are silent. Use when the moral weight is in the quest's resolution rather than in what the player killed getting there.",
        },
      },
      required: ["kind", "delta"],
    },
    {
      ...
      properties: {
        ...
        faction: {
          type: "string",
          description:
            "Faction key, matching MobileData.faction and an entry in scripts/data/factions.json.",
        },
        delta: {
          type: "number",
          description:
            "Shift standing with that faction, clamped to [-1000, 1000]. The faction's declared enemies move the opposite way, scaled by its opposition_ratio. Combat can only LOWER standing, so this and the reputation dialogue effect are the only ways it rises. 50 is Accepted, 200 Honored, 500 Revered.",
        },
      },
