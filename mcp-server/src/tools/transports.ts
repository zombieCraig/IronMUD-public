export const transportToolDefinitions = [
  ...
  },
  {
    ...
    description:
      "Create a new transport (elevator, bus, train, ferry, or airship). Requires an interior room that serves as the vehicle cabin.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Delete a transport. Automatically cleans up any exits if transport is currently connected to a stop.",
    inputSchema: {
      type: "object",
      properties: {
        id: { type: "string", description: "Transport UUID or vnum" },
  ...
  },
  {
    ...
    description:
      "Add a stop to a transport. Each stop defines a room and the exit direction used to enter the transport from that room.",
    inputSchema: {
      type: "object",
      properties: {
        ...
        exit_direction: {
          type: "string",
          description:
            "Direction from the stop room to enter the transport (e.g., 'up', 'east', 'elevator')",
        },
      },
  ...
  },
  {
    ...
    description:
      "Connect a transport to a specific stop (creates bidirectional exits between the stop room and interior room). Sets transport state to Stopped.",
    inputSchema: {
      type: "object",
      properties: {
        transport_id: { type: "string", description: "Transport UUID or vnum" },
  ...
  },
  {
    ...
    description:
      "Start on-demand travel to a destination stop. Disconnects from current stop (removes exits), sets state to Moving. The transport tick system handles arrival.",
    inputSchema: {
      type: "object",
      properties: {
        transport_id: { type: "string", description: "Transport UUID or vnum" },
