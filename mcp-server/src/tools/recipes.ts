...
const ingredientSchema = {
  ...
  properties: {
    vnum: {
      type: "string",
      description:
        "Exact item vnum (mutually exclusive with category). Use when only a specific prototype counts.",
    },
    category: {
      type: "string",
      description:
        "Category tag match (e.g. 'flour', 'sticks', 'shells'). For liquids use 'liquid:<type>' (e.g. 'liquid:water').",
    },
    quantity: {
      type: "number",
...
const toolSchema = {
  ...
  properties: {
    ...
    location: {
      ...
      description:
        "Where the tool must be. 'inventory' = carried, 'room' = present in the room (forge, oven), 'either' = both accepted. Default: inventory.",
    },
  },
  required: [],
};
...
export const recipeToolDefinitions = [
  ...
  },
  {
    name: "create_recipe",
    description:
      "Create a new crafting or cooking recipe. Ingredients and tools may each reference items by exact vnum or by category tag (matches any prototype with that category). For liquid ingredients use category 'liquid:<type>' (e.g. 'liquid:water').",
    inputSchema: {
      type: "object",
      properties: {
        vnum: {
          type: "string",
          description:
            "Unique recipe vnum, e.g. 'islands:shell_necklace'. Used as the canonical id.",
        },
        ...
        auto_learn: {
          type: "boolean",
          description:
            "If true, players learn this automatically upon reaching skill_required. If false, they need a trainer or recipe book.",
        },
        ingredients: {
          type: "array",
  ...
  },
  {
    name: "update_recipe",
    description:
      "Update an existing recipe. Any omitted field is left unchanged. Passing a new `ingredients` or `tools` array REPLACES the list (additive updates are not supported).",
    inputSchema: {
      type: "object",
      properties: {
