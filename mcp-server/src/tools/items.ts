...
export const itemToolDefinitions = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        item_type: {
          type: "string",
          enum: ["misc", "armor", "weapon", "container", "liquid_container", "food", "key", "gold", "ammunition", "potion", "wand", "staff", "note", "pen", "tool", "tattoo", "cyberware"],
        },
      },
    },
  },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        cyber_category: { type: "string", description: "Cyberware category (item_type must be 'cyberware'): fashionware, neuralware, cyberoptic, cyberaudio, cyberarm, cyberleg, internal_body, external_body, borgware. Foundations (neural link, cybereye, cyberaudio suite, cyberarm/leg) provide option slots; options of the same category install into them." },
        cyber_foundation: { type: "boolean", description: "True for base units providing option slots (neural link, cybereye, cyberaudio suite, cyberarm, cyberleg)." },
        cyber_option_slots: { type: "number", description: "Option slots a foundation provides. 0 = category default (neuralware 5 / cyberoptic 3 / cyberaudio 3 / cyberarm 4 / cyberleg 3)." },
        cyber_slot_cost: { type: "number", description: "Slots an option consumes in its host foundation. 0 = 1." },
        cyber_humanity_loss: { type: "number", description: "Humanity charged on install (RED tiers 0/2/3/7/14). 0-HL pieces (fashionware) also skip the max-humanity reduction. Borgware reduces max humanity by 4 instead of 2." },
        cyber_paired: { type: "boolean", description: "Option must occupy a slot in BOTH foundations of its category at once (e.g. low-light optics need both cybereyes)." },
        cyber_exclusive_tag: { type: "string", description: "At most one installed piece per non-empty tag — 'speedware' implements the one-speedware rule." },
        extra_descs: {
          type: "array",
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        item_type: { type: "string", enum: ["misc", "armor", "weapon", "container", "liquid_container", "food", "key", "gold", "ammunition", "potion", "wand", "staff", "note", "pen", "tool", "tattoo", "cyberware"], description: "Change item type" },
        keywords: { type: "array", items: { type: "string" } },
        weight: { type: "number" },
        value: { type: "number" },
        categories: {
