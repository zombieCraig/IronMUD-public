...
const KIND_ENUM = ["mob", "obj", "room"] as const;

const FLAGS_HELP =
  "Letter-flag string mapping to trigger types. For mob: b=OnIdle, c=OnCommand, g=OnGreet, " +
...
export const dgProtoToolDefinitions = [
  {
    name: "list_dg_protos",
    description:
      "List every DG trigger prototype in the registry. Returns vnum / name / kind (mob|obj|room) / flags. " +
  ...
  },
  {
    name: "get_dg_proto",
    description:
      "Fetch a single DG trigger prototype by vnum, including its body, flags, and arglist.",
    inputSchema: {
      type: "object",
      properties: {
        vnum: {
  ...
  },
  {
    ...
    description:
      "Create a new DG trigger prototype in the shared registry. The body is parse-checked before save; " +
  ...
  },
  {
    ...
    description:
      "Update an existing DG trigger prototype. Body edits are parse-checked; on save, every live trigger " +
  ...
  },
  {
    ...
    description:
      "Delete a DG trigger prototype. Attached instances are orphaned but their bodies are preserved (matches the in-game `trigger dg proto delete` behaviour).",
    inputSchema: {
      type: "object",
      properties: {
