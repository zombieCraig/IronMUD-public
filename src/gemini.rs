//! Google Gemini AI integration module for IronMUD
//!
...
/// Request types that can be sent to the Gemini background task
#[derive(Debug)]
...
/// Sender handle for game code to send requests to Gemini
pub type GeminiSender = mpsc::UnboundedSender<GeminiRequest>;

/// Run the Gemini integration background task
pub async fn run_gemini_task(
    config: GeminiConfig,
    connections: SharedConnections,
    mut rx: mpsc::UnboundedReceiver<GeminiRequest>,
) {
    info!("Starting Gemini AI integration task");
...
async fn process_request(
    ...
    match request {
        GeminiRequest::HelpMeWrite {
            ...
            match call_gemini_api(config, client, &system_prompt, &user_prompt).await {
...
async fn call_gemini_api(
    ...
    if !status.is_success() {
        ...
        if let Ok(error_response) = serde_json::from_str::<GeminiApiResponse>(&body) {
    ...
    let api_response: GeminiApiResponse = serde_json::from_str(&body)?;
...
// Prompt building functions - same as Claude module
