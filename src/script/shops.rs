...
pub fn register(engine: &mut Engine, db: Arc<Db>) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("shop_will_buy_item", move |mobile_id: String, item_id: String| -> bool {
        ...
        let preset = if !mobile.shop_preset_vnum.is_empty() {
            cloned_db.get_shop_preset_by_vnum(&mobile.shop_preset_vnum).ok().flatten()
        } else {
            None
        };
        ...
        if !effective_categories.is_empty() {
            let item_cats: Vec<String> = item.categories.iter()
                .map(|c| c.to_lowercase())
                .collect();
            let has_match = effective_categories.iter()
                .any(|ec| item_cats.contains(ec));
            if !has_match {
                return false;
            }
        }
        ...
        let min_val = if let Some(ref p) = preset {
            if mobile.shop_min_value > 0 { mobile.shop_min_value } else { p.min_value }
        } else {
            mobile.shop_min_value
        };
        let max_val = if let Some(ref p) = preset {
            if mobile.shop_max_value > 0 { mobile.shop_max_value } else { p.max_value }
        } else {
    ...
    engine.register_fn("get_shop_buys_categories", move |mobile_id: String| -> rhai::Array {
        if let Ok(uuid) = uuid::Uuid::parse_str(&mobile_id) {
            if let Ok(Some(mobile)) = cloned_db.get_mobile_data(&uuid) {
                return mobile.shop_buys_categories.iter()
                    .map(|s| rhai::Dynamic::from(s.clone()))
                    .collect();
            }
        }
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_shop_buys_categories", move |mobile_id: String, cats: rhai::Array| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&mobile_id) {
            match cloned_db.get_mobile_data(&uuid) {
                Ok(Some(mut mobile)) => {
                    mobile.shop_buys_categories = cats.iter()
                        .filter_map(|v| v.clone().into_string().ok())
                        .collect();
                    cloned_db.save_mobile_data(mobile).is_ok()
                }
    ...
    engine.register_fn("get_shop_extra_types", move |mobile_id: String| -> rhai::Array {
        if let Ok(uuid) = uuid::Uuid::parse_str(&mobile_id) {
            if let Ok(Some(mobile)) = cloned_db.get_mobile_data(&uuid) {
                return mobile.shop_extra_types.iter()
                    .map(|s| rhai::Dynamic::from(s.clone()))
                    .collect();
            }
        }
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_shop_extra_types", move |mobile_id: String, types: rhai::Array| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&mobile_id) {
            match cloned_db.get_mobile_data(&uuid) {
                Ok(Some(mut mobile)) => {
                    mobile.shop_extra_types = types.iter()
                        .filter_map(|v| v.clone().into_string().ok())
                        .collect();
                    cloned_db.save_mobile_data(mobile).is_ok()
                }
    ...
    engine.register_fn("get_shop_extra_categories", move |mobile_id: String| -> rhai::Array {
        if let Ok(uuid) = uuid::Uuid::parse_str(&mobile_id) {
            if let Ok(Some(mobile)) = cloned_db.get_mobile_data(&uuid) {
                return mobile.shop_extra_categories.iter()
                    .map(|s| rhai::Dynamic::from(s.clone()))
                    .collect();
            }
        }
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_shop_extra_categories", move |mobile_id: String, cats: rhai::Array| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&mobile_id) {
            match cloned_db.get_mobile_data(&uuid) {
                Ok(Some(mut mobile)) => {
                    mobile.shop_extra_categories = cats.iter()
                        .filter_map(|v| v.clone().into_string().ok())
                        .collect();
                    cloned_db.save_mobile_data(mobile).is_ok()
                }
    ...
    engine.register_fn("get_shop_deny_types", move |mobile_id: String| -> rhai::Array {
        if let Ok(uuid) = uuid::Uuid::parse_str(&mobile_id) {
            if let Ok(Some(mobile)) = cloned_db.get_mobile_data(&uuid) {
                return mobile.shop_deny_types.iter()
                    .map(|s| rhai::Dynamic::from(s.clone()))
                    .collect();
            }
        }
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_shop_deny_types", move |mobile_id: String, types: rhai::Array| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&mobile_id) {
            match cloned_db.get_mobile_data(&uuid) {
                Ok(Some(mut mobile)) => {
                    mobile.shop_deny_types = types.iter()
                        .filter_map(|v| v.clone().into_string().ok())
                        .collect();
                    cloned_db.save_mobile_data(mobile).is_ok()
                }
    ...
    engine.register_fn("get_shop_deny_categories", move |mobile_id: String| -> rhai::Array {
        if let Ok(uuid) = uuid::Uuid::parse_str(&mobile_id) {
            if let Ok(Some(mobile)) = cloned_db.get_mobile_data(&uuid) {
                return mobile.shop_deny_categories.iter()
                    .map(|s| rhai::Dynamic::from(s.clone()))
                    .collect();
            }
        }
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_shop_deny_categories", move |mobile_id: String, cats: rhai::Array| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&mobile_id) {
            match cloned_db.get_mobile_data(&uuid) {
                Ok(Some(mut mobile)) => {
                    mobile.shop_deny_categories = cats.iter()
                        .filter_map(|v| v.clone().into_string().ok())
                        .collect();
                    cloned_db.save_mobile_data(mobile).is_ok()
                }
