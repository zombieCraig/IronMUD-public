...
use std::sync::Arc;
use crate::db::Db;
use crate::{register_bool_flags, BodyPart, ItemData, ItemFlags, ItemType, ItemEffect, ItemLocation, WearLocation, WeaponSkill, DamageType, LiquidType, EffectType};
...
pub fn register(engine: &mut Engine, db: Arc<Db>) {
    ...
    register_bool_flags!(engine, ItemFlags,
        no_drop, no_get, no_remove, invisible, glow, hum, no_sell, unique,
        quest_item, vending, provides_light, fishing_rod, bait, foraging_tool,
        waterproof, provides_warmth, reduces_glare, medical_tool, is_corpse,
        preserves_contents, death_only, atm, broken, plant_pot, lockpick, is_skinned,
        boat
    );
    ...
    engine.register_type_with_name::<ItemData>("ItemData")
        ...
        .register_get("keywords", |i: &mut ItemData| {
            i.keywords.iter().map(|s| rhai::Dynamic::from(s.clone())).collect::<Vec<_>>()
        })
        .register_get("item_type", |i: &mut ItemData| i.item_type.to_display_string().to_string())
        .register_get("wear_locations", |i: &mut ItemData| {
            i.wear_locations.iter()
        ...
        .register_get("damage_type", |i: &mut ItemData| i.damage_type.to_display_string().to_string())
        .register_get("two_handed", |i: &mut ItemData| i.two_handed)
        .register_get("has_damage", |i: &mut ItemData| i.damage_dice_count > 0 && i.damage_dice_sides > 0)
        // Container properties
        .register_get("container_contents", |i: &mut ItemData| {
            i.container_contents.iter().map(|u| rhai::Dynamic::from(u.to_string())).collect::<Vec<_>>()
        })
        ...
        .register_get("liquid_type", |i: &mut ItemData| i.liquid_type.to_display_string().to_string())
        ...
        .register_get("liquid_effects", |i: &mut ItemData| {
            i.liquid_effects.iter().map(|e| rhai::Dynamic::from(e.clone())).collect::<Vec<_>>()
        })
        .register_get("is_liquid_container", |i: &mut ItemData| i.item_type == ItemType::LiquidContainer)
        .register_get("is_empty", |i: &mut ItemData| {
            match i.item_type {
        ...
        .register_get("food_effects", |i: &mut ItemData| {
            i.food_effects.iter().map(|e| rhai::Dynamic::from(e.clone())).collect::<Vec<_>>()
        })
        ...
        .register_get("has_stats", |i: &mut ItemData| {
            i.stat_str != 0 || i.stat_dex != 0 || i.stat_con != 0 ||
            i.stat_int != 0 || i.stat_wis != 0 || i.stat_cha != 0
        })
        // Prototype fields
        ...
        .register_get("vending_stock", |i: &mut ItemData| {
            i.vending_stock.iter().map(|s| rhai::Dynamic::from(s.clone())).collect::<Vec<_>>()
        })
        ...
        .register_get("treats_wound_types", |i: &mut ItemData| {
            i.treats_wound_types.iter().map(|s| rhai::Dynamic::from(s.clone())).collect::<Vec<_>>()
        })
        ...
        .register_get("ranged_type", |i: &mut ItemData| i.ranged_type.clone().unwrap_or_default())
        ...
        .register_get("supported_fire_modes", |i: &mut ItemData| {
            i.supported_fire_modes.iter().map(|s| rhai::Dynamic::from(s.clone())).collect::<Vec<_>>()
        })
        ...
        .register_get("loaded_ammo_effect_type", |i: &mut ItemData| i.loaded_ammo_effect_type.clone())
        .register_get("loaded_ammo_effect_duration", |i: &mut ItemData| i.loaded_ammo_effect_duration as i64)
        .register_get("loaded_ammo_effect_damage", |i: &mut ItemData| i.loaded_ammo_effect_damage as i64)
        // Attachment fields
        .register_get("attachment_slot", |i: &mut ItemData| i.attachment_slot.clone())
        .register_get("attachment_accuracy_bonus", |i: &mut ItemData| i.attachment_accuracy_bonus as i64)
        .register_get("attachment_noise_reduction", |i: &mut ItemData| i.attachment_noise_reduction as i64)
        .register_get("attachment_magazine_bonus", |i: &mut ItemData| i.attachment_magazine_bonus as i64)
        .register_get("attachment_compatible_types", |i: &mut ItemData| {
            i.attachment_compatible_types.iter().map(|s| rhai::Dynamic::from(s.clone())).collect::<Vec<_>>()
        })
        // Gardening fields
        .register_get("plant_prototype_vnum", |i: &mut ItemData| i.plant_prototype_vnum.clone())
    ...
    engine.register_type_with_name::<ItemEffect>("ItemEffect")
        .register_get("effect_type", |e: &mut ItemEffect| e.effect_type.to_display_string().to_string())
        .register_get("magnitude", |e: &mut ItemEffect| e.magnitude as i64)
        .register_get("duration", |e: &mut ItemEffect| e.duration as i64)
        .register_get("script_callback", |e: &mut ItemEffect| e.script_callback.clone().unwrap_or_default());

    // new_effect(effect_type, magnitude, duration) -> ItemEffect
    engine.register_fn("new_effect", |effect_type_str: String, magnitude: i64, duration: i64| {
        ItemEffect {
    ...
    engine.register_fn("get_all_effect_types", || {
        EffectType::all().iter().map(|s| rhai::Dynamic::from(s.to_string())).collect::<Vec<_>>()
    });

    // get_all_liquid_types() -> Array
    engine.register_fn("get_all_liquid_types", || {
        LiquidType::all().iter().map(|s| rhai::Dynamic::from(s.to_string())).collect::<Vec<_>>()
    });
    ...
    engine.register_fn("get_items_in_room", move |room_id: String| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&room_id) {
            cloned_db.get_items_in_room(&uuid)
                .unwrap_or_default()
                .into_iter()
                .map(rhai::Dynamic::from)
                .collect::<Vec<_>>()
    ...
    engine.register_fn("get_items_in_inventory", move |char_name: String| {
        cloned_db.get_items_in_inventory(&char_name)
            .unwrap_or_default()
            .into_iter()
            .map(rhai::Dynamic::from)
            .collect::<Vec<_>>()
    ...
    engine.register_fn("get_equipped_items", move |char_name: String| {
        cloned_db.get_equipped_items(&char_name)
            .unwrap_or_default()
            .into_iter()
            .map(rhai::Dynamic::from)
            .collect::<Vec<_>>()
    ...
    let cloned_db = db.clone();
    engine.register_fn("move_item_to_mobile_inventory", move |item_id: String, mobile_id: String| {
        match (uuid::Uuid::parse_str(&item_id), uuid::Uuid::parse_str(&mobile_id)) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("move_item_to_mobile_equipped", move |item_id: String, mobile_id: String| {
        match (uuid::Uuid::parse_str(&item_id), uuid::Uuid::parse_str(&mobile_id)) {
    ...
    engine.register_fn("get_items_in_mobile_inventory", move |mobile_id: String| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&mobile_id) {
            cloned_db.get_items_in_mobile_inventory(&uuid)
                .unwrap_or_default()
                .into_iter()
                .map(rhai::Dynamic::from)
                .collect::<Vec<_>>()
    ...
    engine.register_fn("get_items_equipped_on_mobile", move |mobile_id: String| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&mobile_id) {
            cloned_db.get_items_equipped_on_mobile(&uuid)
                .unwrap_or_default()
                .into_iter()
                .map(rhai::Dynamic::from)
                .collect::<Vec<_>>()
    ...
    engine.register_fn("search_items", move |keyword: String| {
        cloned_db.search_items(&keyword)
            .unwrap_or_default()
            .into_iter()
            .map(rhai::Dynamic::from)
            .collect::<Vec<_>>()
    ...
    engine.register_fn("set_item_keywords", move |item_id: String, keywords: rhai::Array| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
                item.keywords = keywords.into_iter()
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_item_wear_locations", move |item_id: String, locations: rhai::Array| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
                item.wear_locations = locations.into_iter()
                    .filter_map(|d| d.try_cast::<String>())
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_item_protects", move |item_id: String, parts: rhai::Array| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
                item.protects = parts.into_iter()
                    .filter_map(|d| d.try_cast::<String>())
    ...
    let cloned_db = db.clone();
    engine.register_fn("remove_item_protects", move |item_id: String, body_part: String| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
                if let Some(bp) = BodyPart::from_str(&body_part) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_item_damage", move |item_id: String, dice_count: i64, dice_sides: i64| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_item_damage_type", move |item_id: String, damage_type_str: String| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
    ...
    engine.register_fn("get_all_damage_types", || {
        DamageType::all().iter().map(|s| rhai::Dynamic::from(s.to_string())).collect::<Vec<_>>()
    });
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_item_weapon_skill", move |item_id: String, skill_str: String| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
    ...
    engine.register_fn("get_equipped_weapon_skill", move |char_name: String| -> rhai::Dynamic {
        ...
        for item in equipped {
            ...
            if item.wear_locations.iter().any(|loc| matches!(loc, WearLocation::Wielded)) {
                if let Some(skill) = item.weapon_skill {
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_item_ammo_damage_bonus", move |item_id: String, bonus: i64| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_item_loaded_ammo_bonus", move |item_id: String, bonus: i64| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_item_loaded_ammo_vnum", move |item_id: String, vnum: String| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("get_item_supported_fire_modes", move |item_id: String| -> Vec<rhai::Dynamic> {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(item)) = cloned_db.get_item_data(&uuid) {
                return item.supported_fire_modes.iter()
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_item_supported_fire_modes", move |item_id: String, modes_str: String| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
                let modes: Vec<String> = modes_str.split(|c: char| c == ',' || c == ' ')
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_item_ammo_effect_type", move |item_id: String, effect_type: String| -> bool {
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_item_ammo_effect_duration", move |item_id: String, duration: i64| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_item_ammo_effect_damage", move |item_id: String, damage: i64| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_item_loaded_ammo_effect_type", move |item_id: String, effect_type: String| -> bool {
        let valid = ["fire", "cold", "poison", "acid", ""];
        let et = effect_type.to_lowercase();
        if !valid.contains(&et.as_str()) {
            return false;
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_item_loaded_ammo_effect_duration", move |item_id: String, duration: i64| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_item_loaded_ammo_effect_damage", move |item_id: String, damage: i64| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_item_attachment_slot", move |item_id: String, slot: String| -> bool {
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_item_attachment_accuracy_bonus", move |item_id: String, bonus: i64| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_item_attachment_noise_reduction", move |item_id: String, reduction: i64| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_item_attachment_magazine_bonus", move |item_id: String, bonus: i64| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("get_item_attachment_compatible_types", move |item_id: String| -> Vec<rhai::Dynamic> {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(item)) = cloned_db.get_item_data(&uuid) {
                return item.attachment_compatible_types.iter()
                    .map(|s| rhai::Dynamic::from(s.clone()))
                    .collect();
            }
        }
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_item_attachment_compatible_types", move |item_id: String, types_str: String| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
                let valid_types = ["bow", "crossbow", "firearm"];
                let types: Vec<String> = types_str.split(|c: char| c == ',' || c == ' ')
                    .map(|s| s.trim().to_lowercase())
    ...
    let cloned_db = db.clone();
    engine.register_fn("can_attach_to_weapon", move |attachment_id: String, weapon_id: String| -> rhai::Map {
        let mut result = rhai::Map::new();
        ...
        let att_uuid = match uuid::Uuid::parse_str(&attachment_id) {
            Ok(u) => u,
            Err(_) => {
                result.insert("reason".into(), rhai::Dynamic::from("Invalid attachment ID.".to_string()));
                return result;
            }
        };
        ...
        let attachment = match cloned_db.get_item_data(&att_uuid) {
            Ok(Some(a)) => a,
            _ => {
                result.insert("reason".into(), rhai::Dynamic::from("Attachment not found.".to_string()));
                return result;
            }
        };
        ...
        if attachment.attachment_slot.is_empty() {
            result.insert("reason".into(), rhai::Dynamic::from("That item is not an attachment.".to_string()));
            return result;
        }
        ...
        let ranged_type = match &weapon.ranged_type {
            Some(rt) => rt.clone(),
            None => {
                result.insert("reason".into(), rhai::Dynamic::from("That weapon is not a ranged weapon.".to_string()));
                return result;
            }
        };
        ...
        if !attachment.attachment_compatible_types.is_empty() {
            ...
            if !attachment.attachment_compatible_types.iter().any(|t| t.to_lowercase() == rt_lower) {
                result.insert("reason".into(), rhai::Dynamic::from(
                    format!("That attachment is not compatible with {} weapons.", ranged_type)
                ));
                return result;
            }
        }
        ...
        if weapon.container_max_items > 0 && weapon.container_contents.len() as i32 >= weapon.container_max_items {
            result.insert("reason".into(), rhai::Dynamic::from("That weapon has no free attachment slots.".to_string()));
            return result;
        }
        ...
        for content_id in &weapon.container_contents {
            if let Ok(Some(existing)) = cloned_db.get_item_data(content_id) {
                if existing.attachment_slot == attachment.attachment_slot {
                    result.insert("reason".into(), rhai::Dynamic::from(
                        format!("That weapon already has a {} attached.", attachment.attachment_slot)
                    ));
                    return result;
                }
            }
    ...
    let cloned_db = db.clone();
    engine.register_fn("get_embedded_projectiles", move |mobile_id: String| -> Vec<rhai::Dynamic> {
        if let Ok(uuid) = uuid::Uuid::parse_str(&mobile_id) {
            if let Ok(Some(mobile)) = cloned_db.get_mobile_data(&uuid) {
                return mobile.embedded_projectiles.iter()
                    .map(|s| rhai::Dynamic::from(s.clone()))
                    .collect();
            }
        }
    ...
    let cloned_db = db.clone();
    engine.register_fn("process_arrow_recovery", move |mobile_id: String, corpse_id: String| -> rhai::Map {
        use rand::Rng;
        let mut result = rhai::Map::new();
    ...
    let cloned_db = db.clone();
    engine.register_fn("perform_reload", move |weapon_id: String, char_name: String| -> rhai::Map {
        let mut result = rhai::Map::new();
        ...
        let caliber = match &weapon.caliber {
            Some(c) => c.clone(),
            None => {
                result.insert("message".into(), rhai::Dynamic::from("This weapon has no caliber set.".to_string()));
                return result;
            }
        };
        ...
        if space <= 0 {
            result.insert("message".into(), rhai::Dynamic::from("Your weapon is already fully loaded.".to_string()));
            return result;
        }

        // Search for compatible ammo: ready slot first, then inventory
        let equipped = cloned_db.get_equipped_items(&char_name).unwrap_or_default();
        let ready_items: Vec<ItemData> = equipped.into_iter()
        ...
        for item in ready_items.iter().chain(inventory.iter()) {
            if item.item_type == ItemType::Ammunition {
                if let Some(ref item_cal) = item.caliber {
                    if *item_cal.to_lowercase() == *caliber.to_lowercase() && item.ammo_count > 0 && !item.flags.broken {
        ...
        let ammo = match ammo_source {
            Some(a) => a,
            None => {
                result.insert("message".into(), rhai::Dynamic::from(
                    format!("You have no {} ammunition to load.", caliber)
                ));
                return result;
            }
        };
        ...
        result.insert("message".into(), rhai::Dynamic::from(
            format!("You load {} round{} of {} ammunition.", to_load, if to_load == 1 { "" } else { "s" }, caliber)
        ));
        result
    });
    ...
    let cloned_db = db.clone();
    engine.register_fn("perform_unload", move |weapon_id: String, char_name: String| -> rhai::Map {
        let mut result = rhai::Map::new();
        result.insert("success".into(), rhai::Dynamic::from(false));
        ...
        if weapon.loaded_ammo <= 0 {
            result.insert("message".into(), rhai::Dynamic::from("Your weapon is not loaded.".to_string()));
            return result;
        }
            ...
            match cloned_db.spawn_item_from_prototype(vnum) {
                Ok(Some(mut spawned)) => {
                    ...
                    if cloned_db.save_item_data(spawned).is_err() {
                        result.insert("message".into(), rhai::Dynamic::from("Failed to create ammo item.".to_string()));
                        return result;
                    }
                    true
                }
        ...
        if !ammo_item {
            ...
            let mut generic = ItemData::new(
                ...
                format!("{} round{} of {} ammunition", count, if count == 1 { "" } else { "s" }, caliber),
        ...
        result.insert("message".into(), rhai::Dynamic::from(
            format!("You eject {} round{} of {} ammunition.", count, if count == 1 { "" } else { "s" }, caliber)
        ));
        result
    });
    ...
    engine.register_fn("get_items_in_container", move |container_id: String| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&container_id) {
            cloned_db.get_items_in_container(&uuid)
                .unwrap_or_default()
                .into_iter()
                .map(rhai::Dynamic::from)
                .collect::<Vec<_>>()
    ...
    let cloned_db = db.clone();
    engine.register_fn("move_item_to_container", move |item_id: String, container_id: String| {
        let item_uuid = uuid::Uuid::parse_str(&item_id).ok();
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_container_capacity", move |item_id: String, max_items: i64, max_weight: i64| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
    ...
    engine.register_fn("can_add_to_container", move |container_id: String, item_id: String| {
        ...
        let container_uuid = match uuid::Uuid::parse_str(&container_id) {
            Ok(u) => u,
            Err(_) => { result.insert("reason".into(), "Invalid container ID".into()); return result; }
        };
        let item_uuid = match uuid::Uuid::parse_str(&item_id) {
            Ok(u) => u,
            Err(_) => { result.insert("reason".into(), "Invalid item ID".into()); return result; }
        };

        let container = match cloned_db.get_item_data(&container_uuid) {
            Ok(Some(c)) if c.item_type == ItemType::Container => c,
            _ => { result.insert("reason".into(), "Not a container".into()); return result; }
        };

        let item = match cloned_db.get_item_data(&item_uuid) {
            Ok(Some(i)) => i,
            _ => { result.insert("reason".into(), "Item not found".into()); return result; }
        };
        ...
        if container.container_max_weight > 0 {
            let current_weight: i32 = container.container_contents.iter()
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_liquid", move |item_id: String, liquid_type_str: String, current: i64, max: i64| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("fill_liquid_container", move |container_id: String, source_id: String| {
        let mut result = rhai::Map::new();
        ...
        match (container_uuid, source_uuid) {
            ...
            _ => { result.insert("message".into(), "Invalid item".into()); }
        }
        result
    });
    ...
    let cloned_db = db.clone();
    engine.register_fn("add_liquid_effect", move |item_id: String, effect_type_str: String, magnitude: i64, duration: i64| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_food_properties", move |item_id: String, nutrition: i64, spoil_duration: i64| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("add_food_effect", move |item_id: String, effect_type_str: String, magnitude: i64, duration: i64| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
                if let Some(et) = EffectType::from_str(&effect_type_str) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_item_stat", move |item_id: String, stat_name: String, value: i64| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_item_transport_link", move |item_id: String, transport_id: String| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("spawn_item_to_inventory", move |vnum: String, char_name: String| -> rhai::Dynamic {
        if let Ok(items) = cloned_db.list_all_items() {
            for item in items {
                if item.is_prototype {
                    if let Some(ref item_vnum) = item.vnum {
    ...
    engine.register_fn("get_item_instances_by_vnum", move |vnum: String| {
        cloned_db.get_item_instances_by_vnum(&vnum)
            .unwrap_or_default()
            .into_iter()
            .map(rhai::Dynamic::from)
            .collect::<Vec<_>>()
    ...
    let cloned_db = db.clone();
    engine.register_fn("spawn_gold_in_room", move |amount: i64, room_id: String| -> rhai::Dynamic {
    ...
    let cloned_db = db.clone();
    engine.register_fn("spawn_gold_in_container", move |amount: i64, container_id: String| -> rhai::Dynamic {
        let container_uuid = match uuid::Uuid::parse_str(&container_id) {
            Ok(u) => u,
            Err(_) => return rhai::Dynamic::UNIT,
        };
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_item_flag", move |item_id: String, flag_name: String, value: bool| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
                match flag_name.to_lowercase().as_str() {
                    ...
                    "preserves_contents" | "preservescontents" | "preserves" => item.flags.preserves_contents = value,
    ...
    let cloned_db = db.clone();
    engine.register_fn("player_has_item_flag", move |char_name: String, flag_name: String| -> bool {
            ...
            if let Some(item) = item_dyn.clone().try_cast::<ItemData>() {
                ...
                if !matched {
                    for item_kw in &item.keywords {
                        if item_kw.to_lowercase() == kw_lower
                            || item_kw.to_lowercase().contains(&kw_lower)
                        {
                            matched = true;
                            break;
                        }
    ...
    engine.register_fn("parse_all_syntax", |args: String| {
        ...
        if args_lower == "all" {
            ...
            result.insert(
                "keyword".into(),
                rhai::Dynamic::from(args[4..].to_string()),
            );
        } else {
    ...
    engine.register_fn("get_character_carry_weight", move |char_name: String| -> i64 {
        ...
        for item in &equipped_items {
            if item.item_type == ItemType::Container {
                ...
                let reduction = if item.weight_reduction > 0 { item.weight_reduction } else { 50 };
    ...
    engine.register_fn("get_encumbrance_level", move |char_name: String| -> String {
        ...
        let percent = if max_carry > 0 { (current_weight * 100) / max_carry } else { 0 };
    ...
    engine.register_fn("get_encumbrance_percent", move |char_name: String| -> i64 {
        ...
        if max_carry > 0 { (current_weight * 100) / max_carry } else { 0 }
    });
    ...
    let cloned_db = db.clone();
    engine.register_fn("remove_treats_wound_type", move |item_id: String, wound_type: String| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
                let wound_lower = wound_type.to_lowercase();
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_item_plant_prototype_vnum", move |item_id: String, vnum: String| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_item_fertilizer_duration", move |item_id: String, duration: i64| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_item_treats_infestation", move |item_id: String, treats: String| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            if let Ok(Some(mut item)) = cloned_db.get_item_data(&uuid) {
