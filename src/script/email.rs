...
use crate::email::{
    ...
    normalize_email, send_password_reset_email, send_test_email, send_verification_email,
};
use rhai::Engine;
use std::sync::Arc;
...
pub fn register(engine: &mut Engine, db: Arc<Db>) {
    ...
    engine.register_fn(
        ...
        move |to_address: String, requested_by: String| -> String {
            ...
            audit_email_send(&cloned_db, "test", &requested_by, audit_outcome_for(&result));
            match result {
                Ok(()) => "sent".to_string(),
                Err(e) => e.to_string(),
            }
