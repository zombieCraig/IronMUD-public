...
use crate::claude::{
    AiDescriptionTarget, AiTargetType, ClaudeRequest, ClaudeSender, DescriptionContext, DescriptionType,
};
use crate::gemini::{GeminiRequest, GeminiSender};
use rhai::Engine;
...
pub fn set_claude_sender(engine: &mut Engine, sender: ClaudeSender, connections: SharedConnections) {
    ...
    engine.register_fn("ai_is_enabled", || -> bool { std::env::var("CLAUDE_API_KEY").is_ok() });
    ...
    engine.register_fn(
        "ai_help_write",
        move |connection_id: String, desc_type: String, prompt: String, context: rhai::Map| -> String {
    ...
    engine.register_fn(
        "ai_rephrase",
        move |connection_id: String, desc_type: String, existing_text: String, context: rhai::Map| -> String {
            let conn_id = match uuid::Uuid::parse_str(&connection_id) {
                Ok(id) => id,
                Err(_) => return String::new(),
            };
    ...
    engine.register_fn(
        "ai_set_target",
        move |connection_id: String, target_type: String, entity_id: String, field: String| -> bool {
            let conn_id = match uuid::Uuid::parse_str(&connection_id) {
                Ok(id) => id,
                Err(_) => return false,
            };
...
pub fn set_gemini_sender(engine: &mut Engine, sender: GeminiSender, connections: SharedConnections) {
    ...
    engine.register_fn("ai_is_enabled", || -> bool { std::env::var("GEMINI_API_KEY").is_ok() });
...
pub fn register_stubs(engine: &mut Engine) {
    ...
    engine.register_fn("ai_is_enabled", || -> bool { false });
}
