...
pub fn register(engine: &mut Engine, db: Arc<Db>, connections: SharedConnections) {
    ...
    engine.register_fn(
        "submit_bug_report",
        move |reporter: String, description: String, context: rhai::Map, is_admin: bool| -> String {
            ...
            let bug_context = BugContext {
                ...
                room_title: context
                    ...
                    .and_then(|d| d.clone().try_cast::<String>())
                    .unwrap_or_default(),
                character_level: context
                    .get("character_level")
                    .and_then(|d| d.clone().as_int().ok())
                    .unwrap_or(0) as i32,
                character_class: context
                    .get("character_class")
                    .and_then(|d| d.clone().try_cast::<String>())
                    .unwrap_or_default(),
                character_race: context
                    .get("character_race")
                    .and_then(|d| d.clone().try_cast::<String>())
                    .unwrap_or_default(),
                character_position: context
            ...
            Ok(reports) => {
                ...
                filtered
                    ...
                    .map(|report| {
                        ...
                        map.insert(
                            "status".into(),
                            rhai::Dynamic::from(report.status.to_display_string().to_string()),
                        );
                        map.insert(
                            "priority".into(),
                            rhai::Dynamic::from(report.priority.to_display_string().to_string()),
                        );
    ...
    engine.register_fn(
        "add_bug_note",
        move |ticket: i64, author: String, message: String| -> bool {
            match cloned_db.get_bug_report_by_ticket(ticket) {
                Ok(Some(mut report)) => {
                    let now = std::time::SystemTime::now()
                        .duration_since(std::time::UNIX_EPOCH)
    ...
    engine.register_fn(
        "notify_admins_bug",
        move |reporter: String, ticket: i64, preview: String| {
    ...
    let conns = connections.clone();
    engine.register_fn(
        "notify_player_bug_update",
        move |reporter: String, ticket: i64, status: String| {
            let conns_guard = conns.lock().unwrap();
            for (_id, session) in conns_guard.iter() {
                if let Some(ref char) = session.character {
...
fn bug_report_to_dynamic(report: &BugReport) -> rhai::Dynamic {
    ...
    ctx.insert(
        "room_vnum".into(),
        rhai::Dynamic::from(report.context.room_vnum.clone()),
    );
    ctx.insert(
        "room_title".into(),
        rhai::Dynamic::from(report.context.room_title.clone()),
    );
    ctx.insert(
        "character_level".into(),
        rhai::Dynamic::from(report.context.character_level as i64),
    );
    ctx.insert(
        "character_class".into(),
        rhai::Dynamic::from(report.context.character_class.clone()),
    );
    ctx.insert(
        "character_race".into(),
        rhai::Dynamic::from(report.context.character_race.clone()),
    );
    ctx.insert(
        "character_position".into(),
        rhai::Dynamic::from(report.context.character_position.clone()),
    );
    ...
    ctx.insert(
        "game_time".into(),
        rhai::Dynamic::from(report.context.game_time.clone()),
    );
