...
pub fn register(engine: &mut Engine, db: Arc<Db>) {
    ...
    engine
        .register_type_with_name::<SimulationConfig>("SimulationConfig")
        .register_get("home_room_vnum", |c: &mut SimulationConfig| c.home_room_vnum.clone())
        .register_set("home_room_vnum", |c: &mut SimulationConfig, v: String| {
            c.home_room_vnum = v
        })
        .register_get("work_room_vnum", |c: &mut SimulationConfig| c.work_room_vnum.clone())
        .register_set("work_room_vnum", |c: &mut SimulationConfig, v: String| {
            c.work_room_vnum = v
        })
        .register_get("shop_room_vnum", |c: &mut SimulationConfig| c.shop_room_vnum.clone())
        .register_set("shop_room_vnum", |c: &mut SimulationConfig, v: String| {
            c.shop_room_vnum = v
        })
        .register_get("preferred_food_vnum", |c: &mut SimulationConfig| {
            c.preferred_food_vnum.clone()
        })
        .register_set("preferred_food_vnum", |c: &mut SimulationConfig, v: String| {
            c.preferred_food_vnum = v
        })
        ...
        .register_set("work_start_hour", |c: &mut SimulationConfig, v: i64| {
            c.work_start_hour = v as u8
        })
        .register_get("work_end_hour", |c: &mut SimulationConfig| c.work_end_hour as i64)
        .register_set("work_end_hour", |c: &mut SimulationConfig, v: i64| {
            c.work_end_hour = v as u8
        })
        .register_get("hunger_decay_rate", |c: &mut SimulationConfig| {
            c.hunger_decay_rate as i64
        })
        .register_set("hunger_decay_rate", |c: &mut SimulationConfig, v: i64| {
            c.hunger_decay_rate = v as i32
        })
        .register_get("energy_decay_rate", |c: &mut SimulationConfig| {
            c.energy_decay_rate as i64
        })
        .register_set("energy_decay_rate", |c: &mut SimulationConfig, v: i64| {
            c.energy_decay_rate = v as i32
        })
        .register_get("comfort_decay_rate", |c: &mut SimulationConfig| {
            c.comfort_decay_rate as i64
        })
        .register_set("comfort_decay_rate", |c: &mut SimulationConfig, v: i64| {
            c.comfort_decay_rate = v as i32
        })
        .register_get("low_gold_threshold", |c: &mut SimulationConfig| {
            c.low_gold_threshold as i64
        })
        .register_set("low_gold_threshold", |c: &mut SimulationConfig, v: i64| {
            c.low_gold_threshold = v as i32
        });

    // Register NeedsState type with getters
    engine
        .register_type_with_name::<NeedsState>("NeedsState")
    ...
    engine
        .register_type_with_name::<SimGoal>("SimGoal")
    ...
    engine.register_fn(
        "set_npc_simulation",
        move |mobile_id: String, home_vnum: String, work_vnum: String, shop_vnum: String| -> bool {
    ...
    let cloned_db = db.clone();
    engine.register_fn(
        "set_npc_low_gold_threshold",
        move |mobile_id: String, threshold: i64| -> bool {
            let uuid = match uuid::Uuid::parse_str(&mobile_id) {
                Ok(u) => u,
                Err(_) => return false,
            };
    ...
    engine.register_fn(
        "set_npc_work_hours",
        move |mobile_id: String, start_hour: i64, end_hour: i64| -> bool {
            let uuid = match uuid::Uuid::parse_str(&mobile_id) {
                Ok(u) => u,
                Err(_) => return false,
            };
    ...
    let cloned_db = db.clone();
    engine.register_fn(
        "set_npc_preferred_food",
        move |mobile_id: String, food_vnum: String| -> bool {
            let uuid = match uuid::Uuid::parse_str(&mobile_id) {
                Ok(u) => u,
                Err(_) => return false,
            };
    ...
    engine.register_fn(
        "set_npc_decay_rate",
        move |mobile_id: String, need_name: String, rate: i64| -> bool {
            let uuid = match uuid::Uuid::parse_str(&mobile_id) {
                Ok(u) => u,
                Err(_) => return false,
            };
    ...
    engine.register_fn("get_npc_needs", move |mobile_id: String| -> rhai::Dynamic {
        ...
        match mobile.needs {
            Some(ref needs) => {
                ...
                map.insert(
                    "goal".into(),
                    rhai::Dynamic::from(needs.current_goal.to_display_string()),
                );
    ...
    engine.register_fn("get_npc_simulation_config", move |mobile_id: String| -> rhai::Dynamic {
        ...
        match mobile.simulation {
            Some(ref config) => {
                ...
                map.insert(
                    "home_room_vnum".into(),
                    rhai::Dynamic::from(config.home_room_vnum.clone()),
                );
                map.insert(
                    "work_room_vnum".into(),
                    rhai::Dynamic::from(config.work_room_vnum.clone()),
                );
                map.insert(
                    "shop_room_vnum".into(),
                    rhai::Dynamic::from(config.shop_room_vnum.clone()),
                );
                map.insert(
                    "preferred_food_vnum".into(),
                    rhai::Dynamic::from(config.preferred_food_vnum.clone()),
                );
                map.insert("work_pay".into(), rhai::Dynamic::from(config.work_pay as i64));
                map.insert(
                    "work_start_hour".into(),
                    rhai::Dynamic::from(config.work_start_hour as i64),
                );
                map.insert("work_end_hour".into(), rhai::Dynamic::from(config.work_end_hour as i64));
                map.insert(
                    "hunger_decay_rate".into(),
                    rhai::Dynamic::from(config.hunger_decay_rate as i64),
                );
                map.insert(
                    "energy_decay_rate".into(),
                    rhai::Dynamic::from(config.energy_decay_rate as i64),
                );
                map.insert(
                    "comfort_decay_rate".into(),
                    rhai::Dynamic::from(config.comfort_decay_rate as i64),
                );
                map.insert(
                    "low_gold_threshold".into(),
                    rhai::Dynamic::from(config.low_gold_threshold as i64),
                );
                rhai::Dynamic::from(map)
            }
            None => rhai::Dynamic::UNIT,
    ...
    engine.register_fn(
        "set_npc_need",
        move |mobile_id: String, need_name: String, value: i64| -> bool {
            let uuid = match uuid::Uuid::parse_str(&mobile_id) {
                Ok(u) => u,
                Err(_) => return false,
            };
    ...
    engine.register_fn(
        "boost_npc_need",
        move |mobile_id: String, need_name: String, amount: i64| -> bool {
            let uuid = match uuid::Uuid::parse_str(&mobile_id) {
                Ok(u) => u,
                Err(_) => return false,
            };
