...
pub fn register(engine: &mut Engine, db: Arc<Db>) {
    ...
    engine.register_type_with_name::<GrowthStageDef>("GrowthStageDef")
        .register_get("stage", |s: &mut GrowthStageDef| s.stage.to_display_string().to_string())
    ...
    engine.register_type_with_name::<PlantPrototype>("PlantPrototype")
        ...
        .register_get("keywords", |p: &mut PlantPrototype| {
            p.keywords.iter().map(|s| rhai::Dynamic::from(s.clone())).collect::<Vec<_>>()
        })
        ...
        .register_get("category", |p: &mut PlantPrototype| p.category.to_display_string().to_string())
        .register_get("stages", |p: &mut PlantPrototype| {
            p.stages.iter().map(|s| rhai::Dynamic::from(s.clone())).collect::<Vec<_>>()
        })
        .register_get("preferred_seasons", |p: &mut PlantPrototype| {
            p.preferred_seasons.iter().map(|s| rhai::Dynamic::from(format!("{}", s))).collect::<Vec<_>>()
        })
        .register_get("forbidden_seasons", |p: &mut PlantPrototype| {
            p.forbidden_seasons.iter().map(|s| rhai::Dynamic::from(format!("{}", s))).collect::<Vec<_>>()
        })
        .register_get("water_consumption_per_hour", |p: &mut PlantPrototype| p.water_consumption_per_hour)
        ...
        .register_get("min_skill_to_plant", |p: &mut PlantPrototype| p.min_skill_to_plant as i64)
        .register_set("min_skill_to_plant", |p: &mut PlantPrototype, v: i64| p.min_skill_to_plant = v as i32)
        ...
        .register_set("pest_resistance", |p: &mut PlantPrototype, v: i64| p.pest_resistance = v as i32)
    ...
    engine.register_type_with_name::<PlantInstance>("PlantInstance")
        ...
        .register_get("group_members", |p: &mut PlantInstance| {
            p.group_members.iter().map(|s| rhai::Dynamic::from(s.clone())).collect::<Vec<_>>()
        })
        ...
        .register_get("fertilizer_hours_remaining", |p: &mut PlantInstance| p.fertilizer_hours_remaining)
        .register_get("infestation", |p: &mut PlantInstance| p.infestation.to_display_string().to_string())
        ...
        .register_get("planted_game_month", |p: &mut PlantInstance| p.planted_game_month as i64)
    ...
    engine.register_fn("list_plant_prototypes", move || {
        cloned_db.list_all_plant_prototypes()
            .unwrap_or_default()
            .into_iter()
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_plant_prototype_category", move |id_str: String, cat_str: String| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&id_str) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_plant_prototype_water", move |id_str: String, consumption: f64, capacity: f64| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&id_str) {
            if let Ok(Some(mut proto)) = cloned_db.get_plant_prototype(&uuid) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("add_plant_prototype_season", move |id_str: String, season_str: String, list_type: String| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&id_str) {
            if let Ok(Some(mut proto)) = cloned_db.get_plant_prototype(&uuid) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("remove_plant_prototype_season", move |id_str: String, season_str: String, list_type: String| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&id_str) {
            if let Ok(Some(mut proto)) = cloned_db.get_plant_prototype(&uuid) {
                let season = match season_str.to_lowercase().as_str() {
                    "spring" => Season::Spring,
    ...
    let cloned_db = db.clone();
    engine.register_fn("add_plant_prototype_stage", move |id_str: String, stage_str: String, duration: i64, desc: String, examine: String| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&id_str) {
            if let Ok(Some(mut proto)) = cloned_db.get_plant_prototype(&uuid) {
                if let Some(stage) = GrowthStage::from_str(&stage_str) {
                    ...
                    proto.stages.sort_by_key(|s| {
                        order.iter().position(|o| *o == s.stage).unwrap_or(99)
                    });
                    return cloned_db.save_plant_prototype(proto).is_ok();
                }
            }
    ...
    let cloned_db = db.clone();
    engine.register_fn("remove_plant_prototype_keyword", move |id_str: String, keyword: String| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&id_str) {
            if let Ok(Some(mut proto)) = cloned_db.get_plant_prototype(&uuid) {
                let kw_lower = keyword.to_lowercase();
    ...
    engine.register_fn("get_plants_in_room", move |room_id: String| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&room_id) {
            cloned_db.get_plants_in_room(&uuid)
                .unwrap_or_default()
                .into_iter()
                .map(rhai::Dynamic::from)
                .collect::<Vec<_>>()
    ...
    let cloned_db = db.clone();
    engine.register_fn("create_plant_instance", move |proto_vnum: String, room_id_str: String, planter_name: String, is_potted: bool, pot_item_id_str: String| {
    ...
    engine.register_fn("harvest_plant", move |plant_id: String, skill_level: i64| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&plant_id) {
            if let Ok(Some(mut plant)) = cloned_db.get_plant(&uuid) {
                ...
                let base_yield = proto.harvest_min as f64
                    + health_factor * (proto.harvest_max - proto.harvest_min) as f64;
    ...
    engine.register_fn("get_plant_examine_description", move |plant_id: String| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&plant_id) {
            if let Ok(Some(plant)) = cloned_db.get_plant(&uuid) {
                if let Ok(Some(proto)) = cloned_db.get_plant_prototype_by_vnum(&plant.prototype_vnum) {
                    let examine = proto.get_stage_def(&plant.stage)
                        .map(|s| s.examine_desc.clone())
                        .unwrap_or_else(|| format!("A {} in the {} stage.", proto.name, plant.stage.to_display_string()));
                    return examine;
                }
            }
        }
    ...
    engine.register_fn("get_all_growth_stages", || {
        GrowthStage::all_names().iter().map(|s| rhai::Dynamic::from(s.to_string())).collect::<Vec<_>>()
    });

    // get_all_plant_categories() -> Array of category name strings
    engine.register_fn("get_all_plant_categories", || {
        PlantCategory::all_names().iter().map(|s| rhai::Dynamic::from(s.to_string())).collect::<Vec<_>>()
    });

    // get_all_infestation_types() -> Array of infestation type name strings
    engine.register_fn("get_all_infestation_types", || {
        InfestationType::all_names().iter().map(|s| rhai::Dynamic::from(s.to_string())).collect::<Vec<_>>()
    });
}
