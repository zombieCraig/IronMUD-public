...
use std::sync::Arc;
use crate::db::Db;
use crate::{TransportData, TransportStop, TransportType, TransportState, TransportSchedule};
...
pub fn register(engine: &mut Engine, db: Arc<Db>) {
    ...
    engine.register_fn("list_all_transports", move || -> Vec<rhai::Dynamic> {
        match cloned_db.list_all_transports() {
            Ok(transports) => transports.into_iter()
    ...
    engine.register_fn("new_transport_stop", |room_id: String, name: String, exit_direction: String| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&room_id) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_transport_vnum", move |transport_id: String, vnum: String| -> bool {
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_transport_name", move |transport_id: String, name: String| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&transport_id) {
            match cloned_db.get_transport(uuid) {
                Ok(Some(mut transport)) => {
                    transport.name = name;
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_transport_type", move |transport_id: String, type_str: String| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&transport_id) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_transport_interior_room", move |transport_id: String, room_id: String| -> bool {
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_transport_travel_time", move |transport_id: String, seconds: i64| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&transport_id) {
            match cloned_db.get_transport(uuid) {
                Ok(Some(mut transport)) => {
    ...
    let cloned_db = db.clone();
    engine.register_fn("add_transport_stop", move |transport_id: String, room_id: String, name: String, exit_direction: String| -> bool {
        if let Ok(transport_uuid) = uuid::Uuid::parse_str(&transport_id) {
            if let Ok(room_uuid) = uuid::Uuid::parse_str(&room_id) {
                match cloned_db.get_transport(transport_uuid) {
                    Ok(Some(mut transport)) => {
    ...
    let cloned_db = db.clone();
    engine.register_fn("remove_transport_stop", move |transport_id: String, index: i64| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&transport_id) {
            match cloned_db.get_transport(uuid) {
                Ok(Some(mut transport)) => {
    ...
    let cloned_db = db.clone();
    engine.register_fn("get_transport_stops", move |transport_id: String| -> Vec<rhai::Dynamic> {
        if let Ok(uuid) = uuid::Uuid::parse_str(&transport_id) {
            match cloned_db.get_transport(uuid) {
                Ok(Some(transport)) => transport.stops.into_iter()
                    .map(rhai::Dynamic::from)
                    .collect(),
                _ => Vec::new(),
            }
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_transport_schedule_gametime", move |transport_id: String, frequency_hours: i64, operating_start: i64, operating_end: i64, dwell_time_secs: i64| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&transport_id) {
            match cloned_db.get_transport(uuid) {
                Ok(Some(mut transport)) => {
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_transport_state", move |transport_id: String, state_str: String| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&transport_id) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_transport_current_stop", move |transport_id: String, index: i64| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&transport_id) {
            match cloned_db.get_transport(uuid) {
                Ok(Some(mut transport)) => {
                    let idx = index as usize;
    ...
    engine.register_fn("is_transport_ondemand", move |transport_id: String| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&transport_id) {
            match cloned_db.get_transport(uuid) {
                Ok(Some(transport)) => matches!(transport.schedule, TransportSchedule::OnDemand),
                _ => false,
            }
        } else {
            false
    ...
    let cloned_db = db.clone();
    engine.register_fn("is_transport_within_operating_hours", move |transport_id: String, hour: i64| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&transport_id) {
            match cloned_db.get_transport(uuid) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("get_transport_schedule_info", move |transport_id: String| -> rhai::Map {
        ...
        if let Ok(uuid) = uuid::Uuid::parse_str(&transport_id) {
            if let Ok(Some(transport)) = cloned_db.get_transport(uuid) {
                match transport.schedule {
                    ...
                    TransportSchedule::GameTime { frequency_hours, operating_start, operating_end, dwell_time_secs } => {
    ...
    let cloned_db = db.clone();
    engine.register_fn("get_transport_time_at_current_state", move |transport_id: String| -> i64 {
        if let Ok(uuid) = uuid::Uuid::parse_str(&transport_id) {
            if let Ok(Some(transport)) = cloned_db.get_transport(uuid) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("connect_transport_to_stop", move |transport_id: String, stop_index: i64| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&transport_id) {
            match cloned_db.get_transport(uuid) {
                Ok(Some(mut transport)) => {
                    ...
                    if cloned_db.set_room_exit(&stop_room_id, &exit_direction, &transport.interior_room_id).is_err() {
                        return false;
                    }
                    ...
                    if cloned_db.set_room_exit(&transport.interior_room_id, interior_exit, &stop_room_id).is_err() {
                        return false;
                    }
    ...
    let cloned_db = db.clone();
    engine.register_fn("disconnect_transport_from_current_stop", move |transport_id: String| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&transport_id) {
            match cloned_db.get_transport(uuid) {
                Ok(Some(mut transport)) => {
    ...
    let cloned_db = db.clone();
    engine.register_fn("start_transport_travel", move |transport_id: String, destination_index: i64| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&transport_id) {
            match cloned_db.get_transport(uuid) {
                Ok(Some(mut transport)) => {
    ...
    let cloned_db = db.clone();
    engine.register_fn("get_stop_index_for_room", move |transport_id: String, room_id: String| -> i64 {
        if let Ok(transport_uuid) = uuid::Uuid::parse_str(&transport_id) {
            if let Ok(room_uuid) = uuid::Uuid::parse_str(&room_id) {
                match cloned_db.get_transport(transport_uuid) {
                    Ok(Some(transport)) => {
    ...
    engine.register_fn("calculate_elevator_travel_time", |from_index: i64, to_index: i64| -> i64 {
...
pub fn register_types(engine: &mut Engine) {
    ...
    engine
        ...
        .register_get("transport_type", |t: &mut TransportData| t.transport_type.to_display_string().to_string())
        .register_get("interior_room_id", |t: &mut TransportData| t.interior_room_id.to_string())
        .register_get("stops", |t: &mut TransportData| {
            t.stops.iter().map(|s| rhai::Dynamic::from(s.clone())).collect::<Vec<_>>()
        })
        .register_get("current_stop_index", |t: &mut TransportData| t.current_stop_index as i64)
