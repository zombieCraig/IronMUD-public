...
use std::sync::Arc;
use crate::db::Db;
use crate::{CharacterData, ItemData, ItemTrigger, ItemTriggerType, MobileData, MobileTrigger, MobileTriggerType, RoomData, RoomTrigger, TriggerType};
...
pub fn register(engine: &mut Engine, db: Arc<Db>, connections: SharedConnections) {
    ...
    engine.register_type_with_name::<RoomTrigger>("RoomTrigger")
        ...
        .register_get("args", |t: &mut RoomTrigger| {
            t.args.iter().map(|s| rhai::Dynamic::from(s.clone())).collect::<rhai::Array>()
        });

    // Helper to get trigger type as string
    engine.register_fn("get_trigger_type", |t: RoomTrigger| {
    ...
    let cloned_db = db.clone();
    engine.register_fn("add_room_trigger", move |room_id: String, trigger_type: String, script_name: String| {
        let room_uuid = match uuid::Uuid::parse_str(&room_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
    ...
    let cloned_db = db.clone();
    engine.register_fn("add_room_trigger_with_args", move |room_id: String, trigger_type: String, script_name: String, args: rhai::Array| {
        let room_uuid = match uuid::Uuid::parse_str(&room_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
    ...
    let cloned_db = db.clone();
    engine.register_fn("add_room_trigger_with_args_from", move |room_id: String, trigger_type: String, script_name: String, parts: rhai::Array, start_idx: i64| {
        let room_uuid = match uuid::Uuid::parse_str(&room_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
        ...
        let string_args: Vec<String> = parts.into_iter()
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_trigger_enabled", move |room_id: String, index: i64, enabled: bool| {
        let room_uuid = match uuid::Uuid::parse_str(&room_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_trigger_interval", move |room_id: String, index: i64, interval_secs: i64| {
        let room_uuid = match uuid::Uuid::parse_str(&room_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
    ...
    engine.register_fn("test_room_trigger", move |room_id: String, trigger_index: i64, connection_id: String| {
        ...
        if idx >= room.triggers.len() {
            result_map.insert("success".into(), false.into());
            result_map.insert("error".into(), format!("Trigger index {} out of range (0-{})", idx, room.triggers.len().saturating_sub(1)).into());
            return result_map;
        }
        ...
        if trigger.script_name.starts_with('@') {
            ...
            let result = execute_room_template(
                template_name,
                &trigger.args,
                &room_uuid,
                &cloned_conns,
            ...
            result_map.insert("result".into(), format!("Template @{} executed: {}", template_name, result).into());
        ...
        if !is_valid_script_name(&trigger.script_name) {
            tracing::warn!("[SECURITY] Blocked trigger with invalid script_name: {}", trigger.script_name);
            result_map.insert("success".into(), false.into());
        ...
        trigger_engine.register_type_with_name::<CharacterData>("CharacterData")
        ...
        trigger_engine.register_type_with_name::<RoomData>("RoomData")
    ...
    let cloned_db = db.clone();
    let cloned_conns = connections.clone();
    engine.register_fn("fire_room_trigger", move |room_id: String, trigger_type: String, connection_id: String, context: rhai::Map| {
        let room_uuid = match uuid::Uuid::parse_str(&room_id) {
            Ok(u) => u,
        ...
        for trigger in &room.triggers {
            ...
            trigger_engine.register_fn("broadcast_to_room", move |rid: String, message: String, exclude: String| {
                if let Ok(room_uuid) = uuid::Uuid::parse_str(&rid) {
                    crate::broadcast_to_room(&conns2, room_uuid, message, if exclude.is_empty() { None } else { Some(&exclude) });
                }
            });
            ...
            trigger_engine.register_fn("set_room_flag", move |room_id: String, flag_name: String, value: bool| {
            ...
            match trigger_engine.compile(&script_content) {
                Ok(ast) => {
                    ...
                    match trigger_engine.call_fn::<rhai::Dynamic>(&mut scope, &ast, "run_trigger", (room_id.clone(), connection_id.clone(), context.clone())) {
                        Ok(result) => {
    ...
    engine.register_type_with_name::<ItemTrigger>("ItemTrigger")
    ...
    engine.register_fn("get_item_trigger_type", |t: ItemTrigger| {
        match t.trigger_type {
    ...
    let cloned_db = db.clone();
    engine.register_fn("add_item_trigger", move |item_id: String, trigger_type: String, script_name: String| {
        let item_uuid = match uuid::Uuid::parse_str(&item_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
    ...
    // Used for templates like @say_greeting with arguments
    let cloned_db = db.clone();
    engine.register_fn("add_item_trigger_with_args", move |item_id: String, trigger_type: String, script_name: String, args: rhai::Array| {
        let item_uuid = match uuid::Uuid::parse_str(&item_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_item_trigger_enabled", move |item_id: String, index: i64, enabled: bool| {
        let item_uuid = match uuid::Uuid::parse_str(&item_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_item_trigger_chance", move |item_id: String, index: i64, chance: i64| {
        let item_uuid = match uuid::Uuid::parse_str(&item_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
    ...
    let cloned_db = db.clone();
    let cloned_conns = connections.clone();
    engine.register_fn("test_item_trigger", move |item_id: String, trigger_index: i64, connection_id: String| {
        let mut result_map = rhai::Map::new();
        ...
        if idx >= item.triggers.len() {
            result_map.insert("success".into(), false.into());
            result_map.insert("error".into(), format!("Trigger index {} out of range (0-{})", idx, item.triggers.len().saturating_sub(1)).into());
            return result_map;
        }
        ...
        trigger_engine.register_type_with_name::<ItemData>("ItemData")
    ...
    let cloned_db = db.clone();
    let cloned_conns = connections.clone();
    engine.register_fn("fire_item_trigger", move |item_id: String, trigger_type_str: String, connection_id: String, context: rhai::Map| {
        let item_uuid = match uuid::Uuid::parse_str(&item_id) {
            Ok(u) => u,
            Err(_) => return "continue".to_string(),
        };
        ...
        // Find all matching triggers
        let matching_triggers: Vec<_> = item.triggers.iter()
        ...
        for trigger in matching_triggers {
            ...
            if trigger.script_name.starts_with('@') {
                let template_name = &trigger.script_name[1..];
                let result = execute_item_template(
                    template_name,
                    &trigger.args,
                    &connection_id,
                    &cloned_conns,
            ...
            // Validate script name to prevent path traversal
            if !is_valid_script_name(&trigger.script_name) {
                tracing::warn!("[SECURITY] Blocked item trigger with invalid script_name: {}", trigger.script_name);
                continue;
            }
            ...
            match trigger_engine.compile(&script_content) {
                Ok(ast) => {
                    ...
                    match trigger_engine.call_fn::<rhai::Dynamic>(&mut scope, &ast, "run_trigger", (item_id.clone(), connection_id.clone(), context.clone())) {
                        Ok(result) => {
                            let result_str = result.to_string();
                            if result_str == "cancel" {
                                return "cancel".to_string();
    ...
    engine.register_type_with_name::<MobileTrigger>("MobileTrigger")
    ...
    engine.register_fn("get_mobile_trigger_type", |t: MobileTrigger| {
        match t.trigger_type {
    ...
    let cloned_db = db.clone();
    engine.register_fn("add_mobile_trigger", move |mobile_id: String, trigger_type: String, script_name: String| {
        let mobile_uuid = match uuid::Uuid::parse_str(&mobile_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
    ...
    // Used for templates like @say_greeting with arguments
    let cloned_db = db.clone();
    engine.register_fn("add_mobile_trigger_with_args", move |mobile_id: String, trigger_type: String, script_name: String, args: rhai::Array| {
        let mobile_uuid = match uuid::Uuid::parse_str(&mobile_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_mobile_trigger_enabled", move |mobile_id: String, index: i64, enabled: bool| {
        let mobile_uuid = match uuid::Uuid::parse_str(&mobile_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_mobile_trigger_chance", move |mobile_id: String, index: i64, chance: i64| {
        let mobile_uuid = match uuid::Uuid::parse_str(&mobile_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_mobile_trigger_interval", move |mobile_id: String, index: i64, interval_secs: i64| {
        let mobile_uuid = match uuid::Uuid::parse_str(&mobile_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
    ...
    engine.register_fn("get_trigger_script_content", |script_name: String| {
        ...
        if script_name.starts_with('@') {
            result.insert("success".into(), false.into());
            result.insert("content".into(), "".into());
            result.insert("error".into(), "Built-in templates (@) have no script file to view.".into());
            return result;
        }
    ...
    let cloned_db = db.clone();
    let cloned_conns = connections.clone();
    engine.register_fn("test_mobile_trigger", move |mobile_id: String, trigger_index: i64, connection_id: String| {
        let mut result_map = rhai::Map::new();
        ...
        if idx >= mobile.triggers.len() {
            result_map.insert("success".into(), false.into());
            result_map.insert("error".into(), format!("Trigger index {} out of range (0-{})", idx, mobile.triggers.len().saturating_sub(1)).into());
            return result_map;
        }
        ...
        trigger_engine.register_type_with_name::<MobileData>("MobileData")
    ...
    let cloned_db = db.clone();
    let cloned_conns = connections.clone();
    engine.register_fn("fire_mobile_trigger", move |mobile_id: String, trigger_type_str: String, connection_id: String, context: rhai::Map| {
        let mobile_uuid = match uuid::Uuid::parse_str(&mobile_id) {
            Ok(u) => u,
            Err(_) => return "continue".to_string(),
        };
        ...
        // Find all matching triggers
        let matching_triggers: Vec<_> = mobile.triggers.iter()
            .filter(|t| t.trigger_type == trigger_type && t.enabled)
            .collect();
        ...
        for trigger in matching_triggers {
            ...
            // Validate script name to prevent path traversal
            if !is_valid_script_name(&trigger.script_name) {
                tracing::warn!("[SECURITY] Blocked mobile trigger with invalid script_name: {}", trigger.script_name);
                continue;
            }
            ...
            match trigger_engine.compile(&script_content) {
                Ok(ast) => {
                    ...
                    match trigger_engine.call_fn::<rhai::Dynamic>(&mut scope, &ast, "run_trigger", (mobile_id.clone(), connection_id.clone(), context.clone())) {
                        Ok(result) => {
                            let result_str = result.to_string();
                            if result_str == "cancel" {
                                return "cancel".to_string();
    ...
    for trigger in matching_triggers {
        ...
        match trigger_engine.compile(&script_content) {
            Ok(ast) => {
                ...
                match trigger_engine.call_fn::<rhai::Dynamic>(&mut scope, &ast, "run_trigger", (mobile_id_owned, connection_id_owned, rhai_context)) {
                    Ok(result) => {
                        let result_str = result.to_string();
                        if result_str == "cancel" {
                            return "cancel".to_string();
