...
pub fn register(engine: &mut Engine, db: Arc<Db>, connections: SharedConnections) {
    ...
    engine.register_fn("has_mail", move |recipient: String| -> bool {
        cloned_db.get_unread_mail_count(&recipient)
            .map(|count| count > 0)
            .unwrap_or(false)
    });
        ...
        match cloned_db.get_mail_for_recipient(&recipient) {
            Ok(messages) => {
                messages.into_iter().map(|msg| {
                    ...
                    let preview = msg.body.lines().next()
                        .map(|line| if line.len() > 50 { format!("{}...", &line[..47]) } else { line.to_string() })
    ...
    let cloned_db = db.clone();
    engine.register_fn("get_mail_by_index", move |recipient: String, index: i64| -> rhai::Dynamic {
        match cloned_db.get_mail_for_recipient(&recipient) {
            Ok(messages) => {
    ...
    let cloned_db = db.clone();
    engine.register_fn("send_mail", move |sender: String, recipient: String, body: String| -> String {
    ...
    engine.register_fn("notify_mail_received", move |recipient: String, sender: String| -> bool {
