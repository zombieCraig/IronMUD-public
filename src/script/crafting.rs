...
pub fn register(engine: &mut Engine, db: Arc<Db>, state: SharedState) {
    ...
    engine
        .register_type_with_name::<Recipe>("Recipe")
        ...
        .register_get("ingredients", |r: &mut Recipe| {
            r.ingredients
                .iter()
                .map(|i| rhai::Dynamic::from(i.clone()))
                .collect::<Vec<_>>()
        })
    ...
    engine
        .register_type_with_name::<RecipeIngredient>("RecipeIngredient")
    ...
    engine
        .register_type_with_name::<RecipeTool>("RecipeTool")
        ...
        .register_get("location", |t: &mut RecipeTool| match t.location {
        ...
        match cloned_db.get_character_data(&char_name.to_lowercase()) {
            Ok(Some(char)) => {
                ...
                if recipe.auto_learn {
                    let skill_level = char
                        .skills
                        .get(&recipe.skill.to_lowercase())
                        .map(|s| s.level)
                        .unwrap_or(0);
    ...
    engine.register_fn(
        "find_ingredients",
        move |char_name: String, ingredient: RecipeIngredient| -> Vec<rhai::Dynamic> {
            ...
            let is_liquid_ingredient = ingredient
                .category
                .as_ref()
    ...
    engine.register_fn(
        "find_tool",
        move |char_name: String, room_id: String, tool: RecipeTool| -> rhai::Dynamic {
    ...
    let cloned_db = db.clone();
    engine.register_fn(
        "consume_liquid_ingredient",
        move |container_ids: rhai::Array, amount_needed: i64| -> bool {
    ...
    engine.register_fn("get_item_category", move |item_id: String| -> rhai::Dynamic {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            match cloned_db.get_item_data(&uuid) {
                Ok(Some(item)) => match item.categories.first() {
    ...
    engine.register_fn("set_item_category", move |item_id: String, category: String| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            match cloned_db.get_item_data(&uuid) {
                Ok(Some(mut item)) => {
                    item.categories = if category.is_empty() {
                        Vec::new()
                    } else {
                        vec![category.to_lowercase()]
                    };
    ...
    engine.register_fn("get_item_categories", move |item_id: String| -> Vec<rhai::Dynamic> {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            match cloned_db.get_item_data(&uuid) {
                Ok(Some(item)) => item.categories.iter().map(|c| rhai::Dynamic::from(c.clone())).collect(),
                _ => Vec::new(),
            }
        } else {
            Vec::new()
    ...
    engine.register_fn(
        "set_item_categories",
        move |item_id: String, categories: rhai::Array| -> bool {
            if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
                match cloned_db.get_item_data(&uuid) {
                    Ok(Some(mut item)) => {
    ...
    engine.register_fn("get_item_teaches_recipe", move |item_id: String| -> rhai::Dynamic {
        if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
            match cloned_db.get_item_data(&uuid) {
                Ok(Some(item)) => match item.teaches_recipe {
    ...
    let cloned_db = db.clone();
    engine.register_fn(
        "set_item_teaches_recipe",
        move |item_id: String, recipe_id: String| -> bool {
            if let Ok(uuid) = uuid::Uuid::parse_str(&item_id) {
                match cloned_db.get_item_data(&uuid) {
                    Ok(Some(mut item)) => {
    ...
    engine.register_fn(
        ...
        |skill_level: i64, tool_quality: i64, ingredient_quality: i64, difficulty: i64| -> i64 {
            use rand::Rng;
    ...
    engine.register_fn(
        "set_recipe_output",
        move |vnum: String, output_vnum: String, quantity: i64| -> bool {
            if quantity < 1 {
                return false;
            }
            let mut state_guard = cloned_state.lock().unwrap();
    ...
    engine.register_fn(
        ...
        move |vnum: String, item_ref: String, quantity: i64| -> bool {
            if quantity < 1 {
                return false;
            }
            let mut state_guard = cloned_state.lock().unwrap();
    ...
    engine.register_fn(
        "add_recipe_tool",
        move |vnum: String, item_ref: String, location: String| -> bool {
    ...
    engine.register_fn(
        "rename_recipe_vnum",
        move |old_vnum: String, new_vnum: String| -> bool {
