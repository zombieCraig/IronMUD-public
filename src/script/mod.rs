...
use crate::db::Db;
use crate::{AreaData, AreaFlags, AreaPermission, CharacterData, DoorState, ExtraDesc, OnlinePlayer, RoomData, RoomExits, RoomFlags};
...
pub fn register_rhai_functions(engine: &mut Engine, db: Arc<Db>, connections: SharedConnections, state: SharedState) {
    ...
    engine.register_type_with_name::<CharacterData>("CharacterData")
        ...
        .register_set("short_description", |c: &mut CharacterData, val: String| c.short_description = val)
        ...
        .register_get("traits", |c: &mut CharacterData| {
            c.traits.iter().map(|s| rhai::Dynamic::from(s.clone())).collect::<Vec<_>>()
        })
        ...
        .register_set("trait_points", |c: &mut CharacterData, val: i64| c.trait_points = val as i32)
        .register_get("creation_complete", |c: &mut CharacterData| c.creation_complete)
        .register_set("creation_complete", |c: &mut CharacterData, val: bool| c.creation_complete = val)
        ...
        .register_set("max_thirst", |c: &mut CharacterData, val: i64| c.max_thirst = val as i32)
        ...
        .register_set("max_hunger", |c: &mut CharacterData, val: i64| c.max_hunger = val as i32)
        ...
        .register_set("must_change_password", |c: &mut CharacterData, val: bool| c.must_change_password = val)
        // Builder mode: show room flags
        .register_get("show_room_flags", |c: &mut CharacterData| c.show_room_flags)
        .register_set("show_room_flags", |c: &mut CharacterData, val: bool| c.show_room_flags = val)
        // Stamina system
        ...
        .register_set("max_stamina", |c: &mut CharacterData, val: i64| c.max_stamina = val as i32)
        // Mana system
        ...
        .register_set("max_breath", |c: &mut CharacterData, val: i64| c.max_breath = val as i32)
        // Character stats
        ...
        // Group/Party system
        .register_get("following", |c: &mut CharacterData| c.following.clone().unwrap_or_default())
        ...
        .register_set("following_mobile_id", |c: &mut CharacterData, val: String| {
            c.following_mobile_id = if val.is_empty() { None } else { uuid::Uuid::parse_str(&val).ok() };
        })
        ...
        .register_set("cold_exposure", |c: &mut CharacterData, val: i64| c.cold_exposure = val as i32)
        .register_get("heat_exposure", |c: &mut CharacterData| c.heat_exposure as i64)
        .register_set("heat_exposure", |c: &mut CharacterData, val: i64| c.heat_exposure = val as i32)
        .register_get("has_hypothermia", |c: &mut CharacterData| c.has_hypothermia)
        .register_set("has_hypothermia", |c: &mut CharacterData, val: bool| c.has_hypothermia = val)
        .register_get("has_frostbite", |c: &mut CharacterData| {
            c.has_frostbite.iter().map(|bp| rhai::Dynamic::from(format!("{:?}", bp))).collect::<Vec<_>>()
        })
        .register_set("has_frostbite", |c: &mut CharacterData, val: rhai::Array| {
            use crate::BodyPart;
            c.has_frostbite = val.into_iter().filter_map(|d| {
        ...
        .register_set("is_unconscious", |c: &mut CharacterData, val: bool| c.is_unconscious = val)
        .register_get("bleedout_rounds_remaining", |c: &mut CharacterData| c.bleedout_rounds_remaining as i64)
        .register_set("bleedout_rounds_remaining", |c: &mut CharacterData, val: i64| c.bleedout_rounds_remaining = val as i32)
        .register_get("has_heat_exhaustion", |c: &mut CharacterData| c.has_heat_exhaustion)
        .register_set("has_heat_exhaustion", |c: &mut CharacterData, val: bool| c.has_heat_exhaustion = val)
        .register_get("has_heat_stroke", |c: &mut CharacterData| c.has_heat_stroke)
        .register_set("has_heat_stroke", |c: &mut CharacterData, val: bool| c.has_heat_stroke = val)
        ...
        .register_set("illness_progress", |c: &mut CharacterData, val: i64| c.illness_progress = val as i32)
        ...
        .register_set("tour_origin_room", |c: &mut CharacterData, val: String| {
            c.tour_origin_room = if val.is_empty() { None } else { uuid::Uuid::parse_str(&val).ok() };
        })
    ...
    engine.register_fn("new_character", |name: String, password_hash: String, room_id: String| {
        CharacterData {
            name,
            password_hash,
    ...
    engine.register_type_with_name::<RoomExits>("RoomExits")
        .register_get("north", |e: &mut RoomExits| e.north.map(|u| u.to_string()).unwrap_or_default())
        .register_get("east", |e: &mut RoomExits| e.east.map(|u| u.to_string()).unwrap_or_default())
        .register_get("south", |e: &mut RoomExits| e.south.map(|u| u.to_string()).unwrap_or_default())
        .register_get("west", |e: &mut RoomExits| e.west.map(|u| u.to_string()).unwrap_or_default())
        .register_get("up", |e: &mut RoomExits| e.up.map(|u| u.to_string()).unwrap_or_default())
        .register_get("down", |e: &mut RoomExits| e.down.map(|u| u.to_string()).unwrap_or_default())
        .register_get("out", |e: &mut RoomExits| e.out.map(|u| u.to_string()).unwrap_or_default())
    ...
    engine.register_type_with_name::<RoomFlags>("RoomFlags")
        .register_get("dark", |f: &mut RoomFlags| f.dark)
        .register_get("combat_zone", |f: &mut RoomFlags| {
            f.combat_zone.map(|z| z.to_display_string().to_string()).unwrap_or_else(|| "inherit".to_string())
        })
    ...
    engine.register_type_with_name::<AreaData>("AreaData")
        ...
        .register_get("owner", |a: &mut AreaData| {
            a.owner.clone().unwrap_or_default()
        })
        .register_get("permission_level", |a: &mut AreaData| {
            match a.permission_level {
        ...
        .register_get("city_forage_table", |a: &mut AreaData| {
            a.city_forage_table.iter().map(|entry| {
                let mut map = rhai::Map::new();
        ...
        .register_get("wilderness_forage_table", |a: &mut AreaData| {
            a.wilderness_forage_table.iter().map(|entry| {
                let mut map = rhai::Map::new();
                map.insert("vnum".into(), rhai::Dynamic::from(entry.vnum.clone()));
                map.insert("min_skill".into(), rhai::Dynamic::from(entry.min_skill as i64));
                map.insert("rarity".into(), rhai::Dynamic::from(entry.rarity.clone()));
        ...
        .register_get("shallow_water_forage_table", |a: &mut AreaData| {
            a.shallow_water_forage_table.iter().map(|entry| {
                let mut map = rhai::Map::new();
                map.insert("vnum".into(), rhai::Dynamic::from(entry.vnum.clone()));
                map.insert("min_skill".into(), rhai::Dynamic::from(entry.min_skill as i64));
                map.insert("rarity".into(), rhai::Dynamic::from(entry.rarity.clone()));
        ...
        .register_get("deep_water_forage_table", |a: &mut AreaData| {
            a.deep_water_forage_table.iter().map(|entry| {
                let mut map = rhai::Map::new();
                map.insert("vnum".into(), rhai::Dynamic::from(entry.vnum.clone()));
                map.insert("min_skill".into(), rhai::Dynamic::from(entry.min_skill as i64));
                map.insert("rarity".into(), rhai::Dynamic::from(entry.rarity.clone()));
        ...
        .register_get("underwater_forage_table", |a: &mut AreaData| {
            a.underwater_forage_table.iter().map(|entry| {
                let mut map = rhai::Map::new();
                map.insert("vnum".into(), rhai::Dynamic::from(entry.vnum.clone()));
                map.insert("min_skill".into(), rhai::Dynamic::from(entry.min_skill as i64));
                map.insert("rarity".into(), rhai::Dynamic::from(entry.rarity.clone()));
        ...
        .register_get("immigration_room_vnum", |a: &mut AreaData| a.immigration_room_vnum.clone())
        .register_get("immigration_name_pool", |a: &mut AreaData| a.immigration_name_pool.clone())
        .register_get("immigration_visual_profile", |a: &mut AreaData| a.immigration_visual_profile.clone())
        .register_get("migration_interval_days", |a: &mut AreaData| a.migration_interval_days as i64)
        .register_get("migration_max_per_check", |a: &mut AreaData| a.migration_max_per_check as i64)
    ...
    engine.register_type_with_name::<RoomData>("RoomData")
        ...
        .register_get("extra_descs", |r: &mut RoomData| {
            r.extra_descs.iter().map(|e| rhai::Dynamic::from(e.clone())).collect::<Vec<_>>()
        })
        .register_get("vnum", |r: &mut RoomData| r.vnum.clone().unwrap_or_default())
        .register_get("area_id", |r: &mut RoomData| r.area_id.map(|u| u.to_string()).unwrap_or_default())
        .register_get("spring_desc", |r: &mut RoomData| r.spring_desc.clone().unwrap_or_default())
        .register_get("summer_desc", |r: &mut RoomData| r.summer_desc.clone().unwrap_or_default())
        .register_get("autumn_desc", |r: &mut RoomData| r.autumn_desc.clone().unwrap_or_default())
        .register_get("winter_desc", |r: &mut RoomData| r.winter_desc.clone().unwrap_or_default())
        .register_get("dynamic_desc", |r: &mut RoomData| r.dynamic_desc.clone().unwrap_or_default())
    ...
    engine.register_fn("new_room", |id: String, title: String, description: String| {
        RoomData {
    ...
    engine.register_fn("get_available_exits", |room: RoomData| {
        ...
        if room.exits.north.is_some() { exits.push(rhai::Dynamic::from("north")); }
        if room.exits.east.is_some() { exits.push(rhai::Dynamic::from("east")); }
        if room.exits.south.is_some() { exits.push(rhai::Dynamic::from("south")); }
        if room.exits.west.is_some() { exits.push(rhai::Dynamic::from("west")); }
        if room.exits.up.is_some() { exits.push(rhai::Dynamic::from("up")); }
        if room.exits.down.is_some() { exits.push(rhai::Dynamic::from("down")); }
        exits
    });

    // Register DoorState type with getters
    engine.register_type_with_name::<DoorState>("DoorState")
    ...
    engine.register_type_with_name::<OnlinePlayer>("OnlinePlayer")
    ...
    engine.register_fn("get_character_data", move |name: String| {
        cloned_db.get_character_data(&name)
    ...
    engine.register_fn("save_character_data", move |character: CharacterData| {
        cloned_db.save_character_data(character)
            .map_err(|e| Box::new(EvalAltResult::ErrorRuntime(rhai::Dynamic::from(format!("DB Error: {}", e)), Position::NONE)))
    ...
    engine.register_fn("hash_password", move |password: String| {
        cloned_db.hash_password(&password)
            .map_err(|e| Box::new(EvalAltResult::ErrorRuntime(rhai::Dynamic::from(format!("Hashing Error: {}", e)), Position::NONE)))
            .map(rhai::Dynamic::from)
    });

    let cloned_db = db.clone();
    engine.register_fn("verify_password", move |password: String, hash: String| {
        cloned_db.verify_password(&password, &hash)
            .map_err(|e| Box::new(EvalAltResult::ErrorRuntime(rhai::Dynamic::from(format!("Verification Error: {}", e)), Position::NONE)))
            .map(rhai::Dynamic::from)
    });
    ...
    let conns = connections.clone();
    engine.register_fn("broadcast_to_room", move |room_id: String, message: String, exclude_name: String| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&room_id) {
            let exclude = if exclude_name.is_empty() { None } else { Some(exclude_name.as_str()) };
    ...
    let conns = connections.clone();
    engine.register_fn("broadcast_to_room_awake", move |room_id: String, message: String, exclude_name: String| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&room_id) {
            let exclude = if exclude_name.is_empty() { None } else { Some(exclude_name.as_str()) };
    ...
    let conns = connections.clone();
    engine.register_fn("broadcast_to_room_dreaming", move |room_id: String, awake_msg: String, sleeping_msg: String, exclude_name: String| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&room_id) {
            let exclude = if exclude_name.is_empty() { None } else { Some(exclude_name.as_str()) };
    ...
    let conns = connections.clone();
    engine.register_fn("broadcast_to_room_except", move |room_id: String, connection_id: String, message: String| {
    ...
    let conns = connections.clone();
    engine.register_fn("set_player_character", move |connection_id: String, character: CharacterData| {
    ...
    engine.register_fn("kick_player", move |target_name: String, reason: String, admin_name: String| {
        // Find player by name
            ...
            if let Some((character, room_id, sender)) = char_info {
                ...
                // Notify chat integrations (Matrix/Discord)
                {
                    ...
                    if let Some(ref chat_tx) = world.chat_sender {
                        let _ = chat_tx.send(crate::chat::ChatMessage::Broadcast(
                            format!("{} has been removed from the realm.", char_name),
                        ));
                    }
                }
    ...
    engine.register_fn("schedule_shutdown", move |delay_seconds: i64, reason: String, admin_name: String| {
        if delay_seconds < 0 {
            return false;
        }
    ...
    let conns = connections.clone();
    engine.register_fn("send_client_message", move |connection_id_str: String, message: String| {
    ...
    engine.register_fn("get_available_commands", move |connection_id: String| {
        ...
        let (is_logged_in, is_builder, is_admin) = if let Ok(uuid) = uuid::Uuid::parse_str(&connection_id) {
            ...
            conns.get(&uuid)
                .map(|s| {
                    ...
                    let (builder, admin) = s.character.as_ref()
        ...
        commands.sort_by(|a, b| {
            let a_name = a.clone().try_cast::<rhai::Map>()
                .and_then(|m| m.get("name").cloned())
                .and_then(|d| d.try_cast::<String>())
                .unwrap_or_default();
            let b_name = b.clone().try_cast::<rhai::Map>()
                .and_then(|m| m.get("name").cloned())
                .and_then(|d| d.try_cast::<String>())
                .unwrap_or_default();
            a_name.cmp(&b_name)
    ...
    engine.register_fn("set_alias", move |connection_id: String, alias_name: String, expansion: String| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&connection_id) {
    ...
    engine.register_fn("get_olc_buffer", move |connection_id: String| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&connection_id) {
            let conns = conns.lock().unwrap();
            if let Some(session) = conns.get(&uuid) {
                return session.olc_buffer.iter()
                    .map(|s| rhai::Dynamic::from(s.clone()))
                    .collect::<Vec<_>>();
            }
        }
    ...
    engine.register_fn("set_olc_buffer", move |connection_id: String, lines: rhai::Array| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&connection_id) {
            let mut conns = conns.lock().unwrap();
            if let Some(session) = conns.get_mut(&uuid) {
                session.olc_buffer = lines.into_iter()
    ...
    let conns = connections.clone();
    engine.register_fn("set_olc_extra_keywords", move |connection_id: String, keywords: rhai::Array| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&connection_id) {
            let mut conns = conns.lock().unwrap();
            if let Some(session) = conns.get_mut(&uuid) {
                session.olc_extra_keywords = keywords.into_iter()
                    .filter_map(|d| d.try_cast::<String>())
                    .collect();
                return true;
            }
    ...
    engine.register_fn("get_olc_extra_keywords", move |connection_id: String| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&connection_id) {
            let conns = conns.lock().unwrap();
            if let Some(session) = conns.get(&uuid) {
                return session.olc_extra_keywords.iter()
                    .map(|s| rhai::Dynamic::from(s.clone()))
                    .collect::<Vec<_>>();
            }
        }
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_room_water_type", move |room_id: String, type_str: String| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&room_id) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("add_catch_entry", move |room_id: String, vnum: String, weight: i64, min_skill: i64, rarity: String| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&room_id) {
            match cloned_db.get_room_data(&uuid) {
                Ok(Some(mut room)) => {
    ...
    engine.register_fn("get_catch_table", move |room_id: String| -> Vec<rhai::Dynamic> {
        if let Ok(uuid) = uuid::Uuid::parse_str(&room_id) {
            match cloned_db.get_room_data(&uuid) {
                Ok(Some(room)) => {
                    room.catch_table.iter().map(|entry| {
                        let mut map = rhai::Map::new();
                        map.insert("vnum".into(), rhai::Dynamic::from(entry.vnum.clone()));
    ...
    let cloned_db = db.clone();
    engine.register_fn("select_catch", move |room_id: String, skill_level: i64| -> rhai::Dynamic {
        if let Ok(uuid) = uuid::Uuid::parse_str(&room_id) {
            match cloned_db.get_room_data(&uuid) {
                Ok(Some(room)) => {
                    // Filter by skill level
                    let available: Vec<&crate::CatchEntry> = room.catch_table.iter()
