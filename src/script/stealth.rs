...
pub fn register(engine: &mut Engine, db: Arc<Db>, connections: SharedConnections) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("calculate_stealth_score", move |char_name: String, room_id: String| -> i64 {
        let key = char_name.to_lowercase();
        ...
        let stealth_skill = character.skills.get("stealth")
    ...
    engine.register_fn("calculate_perception_score_player", move |char_name: String| -> i64 {
        ...
        let stealth_skill = character.skills.get("stealth")
            .map(|s| s.level as i64)
            .unwrap_or(0);
        let tracking_skill = character.skills.get("tracking")
            .map(|s| s.level as i64)
            .unwrap_or(0);
        ...
        if character.active_buffs.iter().any(|b| b.effect_type == EffectType::DetectInvisible) {
            score += 30;
        }
    ...
    let cloned_db = db.clone();
    engine.register_fn("stealth_check_vs_player", move |stealther: String, observer: String, room_id: String| -> bool {
        ...
        if stealther_char.active_buffs.iter().any(|b| b.effect_type == EffectType::Invisibility) {
            if let Ok(Some(observer_char)) = cloned_db.get_character_data(&observer_key) {
                if !observer_char.active_buffs.iter().any(|b| b.effect_type == EffectType::DetectInvisible) {
        ...
        let stealth_skill = stealther_char.skills.get("stealth")
            .map(|s| s.level as i64)
            .unwrap_or(0);
        ...
        let obs_stealth = observer_char.skills.get("stealth")
            .map(|s| s.level as i64)
            .unwrap_or(0);
        let obs_tracking = observer_char.skills.get("tracking")
            .map(|s| s.level as i64)
            .unwrap_or(0);
    ...
    let cloned_db = db.clone();
    engine.register_fn("stealth_check_vs_mobile", move |stealther: String, mobile_id: String, room_id: String| -> bool {
        let stealther_key = stealther.to_lowercase();
    ...
    let cloned_db = db.clone();
    engine.register_fn("get_theft_cooldown", move |char_name: String, target_id: String| -> i64 {
        let key = char_name.to_lowercase();
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_theft_cooldown", move |char_name: String, target_id: String, duration: i64| -> bool {
        let key = char_name.to_lowercase();
        if let Ok(Some(mut character)) = cloned_db.get_character_data(&key) {
            let now = std::time::SystemTime::now()
                .duration_since(std::time::SystemTime::UNIX_EPOCH)
    ...
    let cloned_db = db.clone();
    let cloned_conns = connections.clone();
    engine.register_fn("set_hunting_target", move |char_name: String, target_name: String| -> bool {
        let key = char_name.to_lowercase();
        if let Ok(Some(mut character)) = cloned_db.get_character_data(&key) {
    ...
    let cloned_db = db.clone();
    let cloned_conns = connections.clone();
    engine.register_fn("set_envenomed_charges", move |char_name: String, charges: i64| -> bool {
        let key = char_name.to_lowercase();
        if let Ok(Some(mut character)) = cloned_db.get_character_data(&key) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("record_departure", move |room_id: String, name: String, direction: String, sneaking: bool| -> bool {
    ...
    let cloned_db = db.clone();
    engine.register_fn("get_recent_departures", move |room_id: String, tracker_skill: i64| -> rhai::Array {
        let room_uuid = match uuid::Uuid::parse_str(&room_id) {
            Ok(u) => u,
            Err(_) => return vec![],
        };
        ...
        for dep in &room.recent_departures {
            ...
            if dep.is_sneaking && tracker_skill < 5 {
                ...
                let has_blood = room.blood_trails.iter().any(|t| {
                    now - t.timestamp < 300 && t.name.to_lowercase() == dep_name_lower
                });
                if !has_blood {
                    continue;
                }
    ...
    let cloned_db = db.clone();
    engine.register_fn("clear_departures_for", move |room_id: String, char_name: String| -> bool {
        let room_uuid = match uuid::Uuid::parse_str(&room_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
    ...
    let cloned_db = db.clone();
    engine.register_fn("find_target_direction", move |room_id: String, target_name: String| -> String {
        let room_uuid = match uuid::Uuid::parse_str(&room_id) {
            Ok(u) => u,
        ...
        for dep in &room.recent_departures {
            if now - dep.timestamp >= 900 { continue; }
    ...
    let cloned_db = db.clone();
    engine.register_fn("add_room_trap", move |room_id: String, trap_type: String, owner: String,
                        damage: i64, detect_diff: i64, disarm_diff: i64, charges: i64, effect: String| -> bool {
        let room_uuid = match uuid::Uuid::parse_str(&room_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
    ...
    engine.register_fn("get_room_traps", move |room_id: String| -> rhai::Array {
        ...
        for (i, trap) in room.traps.iter().enumerate() {
            ...
            map.insert("detect_difficulty".into(), rhai::Dynamic::from(trap.detect_difficulty as i64));
            map.insert("disarm_difficulty".into(), rhai::Dynamic::from(trap.disarm_difficulty as i64));
    ...
    let cloned_db = db.clone();
    engine.register_fn("add_blood_trail", move |room_id: String, name: String, severity: i64, direction: String| -> bool {
        let room_uuid = match uuid::Uuid::parse_str(&room_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
        if let Ok(Some(mut room)) = cloned_db.get_room_data(&room_uuid) {
            ...
            if let Some(existing) = room.blood_trails.iter_mut().find(|t| t.name.to_lowercase() == name_lower) {
    ...
    engine.register_fn("get_blood_trails", move |room_id: String| -> rhai::Array {
        ...
        for trail in &room.blood_trails {
            ...
            map.insert("direction".into(), rhai::Dynamic::from(
                trail.direction.clone().unwrap_or_default()
            ));
            results.push(rhai::Dynamic::from(map));
        }
        results
