...
pub fn register(engine: &mut Engine, connections: SharedConnections) {
    ...
    engine.register_fn("try_enter_modern_editor", move |connection_id: String| -> bool {
        ...
        let ansi_ok = (session.telnet_state.mtts_flags & MTTS_ANSI) != 0 || session.colors_enabled;
