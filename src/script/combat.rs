...
use uuid::Uuid;
use crate::db::Db;
use crate::{BodyPart, CombatDistance, CombatState, CombatTarget, CombatTargetType, CombatZoneType, OngoingEffect, WeaponSkill, Wound, WoundLevel, WoundType};
use crate::{ItemData, ItemType, ItemFlags, ItemLocation, LiquidType, WearLocation, STARTING_ROOM_ID};
...
pub fn register(engine: &mut Engine, db: Arc<Db>) {
    ...
    engine.register_type_with_name::<CombatTarget>("CombatTarget")
        .register_get("target_type", |t: &mut CombatTarget| t.target_type.to_display_string().to_string())
    ...
    engine.register_type_with_name::<CombatState>("CombatState")
        ...
        .register_get("stun_rounds_remaining", |s: &mut CombatState| s.stun_rounds_remaining as i64)
        .register_get("targets", |s: &mut CombatState| {
            s.targets.iter().map(|t| rhai::Dynamic::from(t.clone())).collect::<Vec<_>>()
        })
    ...
    engine.register_fn("get_wound_type_for_damage", |damage_type: String, severity: String| -> String {
    ...
    let cloned_db = db.clone();
    engine.register_fn("get_character_wound_level", move |char_name: String, body_part: String| -> String {
        if let Ok(Some(char)) = cloned_db.get_character_data(&char_name) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("inflict_character_wound", move |char_name: String, body_part: String, wound_type: String, level: String, bleeding: i64| -> bool {
    ...
    let cloned_db = db.clone();
    engine.register_fn("heal_character_wound", move |char_name: String, body_part: String| -> bool {
        let bp = match BodyPart::from_str(&body_part) {
            Some(p) => p,
            None => return false,
        };
    ...
    let cloned_db = db.clone();
    engine.register_fn("get_character_body_part_penalty", move |char_name: String, body_part: String| -> i64 {
        if let Ok(Some(char)) = cloned_db.get_character_data(&char_name) {
            if let Some(bp) = BodyPart::from_str(&body_part) {
                if let Some(wound) = char.wounds.iter().find(|w| w.body_part == bp) {
    ...
    engine.register_fn("get_mobile_wounds", move |mobile_id: String| -> rhai::Array {
        ...
        if let Ok(uuid) = uuid::Uuid::parse_str(&mobile_id) {
            if let Ok(Some(mobile)) = cloned_db.get_mobile_data(&uuid) {
                tracing::debug!("get_mobile_wounds: found mobile {}, wounds count={}", mobile.name, mobile.wounds.len());
    ...
    let cloned_db = db.clone();
    engine.register_fn("get_mobile_wound_level", move |mobile_id: String, body_part: String| -> String {
        if let Ok(uuid) = uuid::Uuid::parse_str(&mobile_id) {
            if let Ok(Some(mobile)) = cloned_db.get_mobile_data(&uuid) {
                if let Some(bp) = BodyPart::from_str(&body_part) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("inflict_mobile_wound", move |mobile_id: String, body_part: String, wound_type: String, level: String, bleeding: i64| -> bool {
        let bp = match BodyPart::from_str(&body_part) {
            Some(p) => p,
            None => return false,
        };
    ...
    let cloned_db = db.clone();
    engine.register_fn("heal_mobile_wound", move |mobile_id: String, body_part: String| -> bool {
        let bp = match BodyPart::from_str(&body_part) {
            Some(p) => p,
            None => return false,
        };
    ...
    let cloned_db = db.clone();
    engine.register_fn("get_mobile_body_part_penalty", move |mobile_id: String, body_part: String| -> i64 {
        if let Ok(uuid) = uuid::Uuid::parse_str(&mobile_id) {
            if let Ok(Some(mobile)) = cloned_db.get_mobile_data(&uuid) {
                if let Some(bp) = BodyPart::from_str(&body_part) {
                    if let Some(wound) = mobile.wounds.iter().find(|w| w.body_part == bp) {
    ...
    engine.register_fn("get_stat_modifier", |stat_value: i64| -> i64 {
        (stat_value - 10) / 2
    });
    ...
    engine.register_fn("calculate_hit_chance", |attacker_skill: i64, attacker_dex_mod: i64, target_dex_mod: i64, target_ac: i64| -> i64 {
    ...
    engine.register_fn("calculate_damage", |dice_count: i64, dice_sides: i64, damage_bonus: i64| -> i64 {
    ...
    let cloned_db = db.clone();
    engine.register_fn("add_wound_bleeding", move |char_name: String, body_part: String, severity: i64| -> bool {
        let bp = match BodyPart::from_str(&body_part) {
            Some(p) => p,
            None => return false,
        };
    ...
    let cloned_db = db.clone();
    engine.register_fn("add_mobile_wound_bleeding", move |mobile_id: String, body_part: String, severity: i64| -> bool {
        let bp = match BodyPart::from_str(&body_part) {
            Some(p) => p,
            None => return false,
        };
    ...
    let cloned_db = db.clone();
    engine.register_fn("escalate_wound_to_severe", move |char_name: String, body_part: String| -> bool {
        let bp = match BodyPart::from_str(&body_part) {
            Some(p) => p,
            None => return false,
        };
    ...
    let cloned_db = db.clone();
    engine.register_fn("escalate_mobile_wound_to_severe", move |mobile_id: String, body_part: String| -> bool {
        tracing::debug!("escalate_mobile_wound_to_severe called: mobile_id={}, body_part={}", mobile_id, body_part);

        let bp = match BodyPart::from_str(&body_part) {
            Some(p) => p,
            None => {
                tracing::debug!("escalate_mobile_wound_to_severe: failed to parse body_part '{}'", body_part);
                return false;
            }
        };
        ...
        let mid = match Uuid::parse_str(&mobile_id) {
            Ok(u) => u,
            Err(e) => {
                tracing::debug!("escalate_mobile_wound_to_severe: failed to parse mobile_id '{}': {}", mobile_id, e);
                return false;
            }
        };

        if let Ok(Some(mut mobile)) = cloned_db.get_mobile_data(&mid) {
            tracing::debug!("escalate_mobile_wound_to_severe: found mobile {}, current wounds={}", mobile.name, mobile.wounds.len());
            // Find existing wound on this body part and escalate to Severe
            for wound in mobile.wounds.iter_mut() {
                if wound.body_part == bp {
                    wound.level = WoundLevel::Severe;
                    let result = cloned_db.save_mobile_data(mobile);
                    tracing::debug!("escalate_mobile_wound_to_severe: escalated existing wound, save result={:?}", result.is_ok());
                    return result.is_ok();
                }
            }
            // No existing wound - create new Severe wound
            ...
            let result = cloned_db.save_mobile_data(mobile);
            tracing::debug!("escalate_mobile_wound_to_severe: created new wound, save result={:?}", result.is_ok());
            return result.is_ok();
        } else {
    ...
    let cloned_db = db.clone();
    engine.register_fn("consume_stamina_for_attack", move |char_name: String, amount: i64| -> bool {
        if let Ok(Some(mut char)) = cloned_db.get_character_data(&char_name) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("get_armor_for_body_part", move |char_name: String, body_part: String| -> rhai::Array {
        let bp = match BodyPart::from_str(&body_part) {
            Some(p) => p,
            None => return vec![],
        };
        ...
        equipped.into_iter()
            .filter(|item| item.protects.contains(&bp)
            ...
            .map(|item| {
                ...
                map.insert("armor_class".into(), Dynamic::from(item.armor_class.unwrap_or(0) as i64));
    ...
    let cloned_db = db.clone();
    engine.register_fn("get_mobile_armor_for_body_part", move |mobile_id: String, body_part: String| -> rhai::Array {
        let bp = match BodyPart::from_str(&body_part) {
            Some(p) => p,
            None => return vec![],
        };
    ...
    let cloned_db = db.clone();
    engine.register_fn("enter_combat", move |char_name: String, target_type: String, target_id: String| -> bool {
    ...
    let cloned_db = db.clone();
    engine.register_fn("remove_combat_target", move |char_name: String, target_id: String| -> bool {
        let tid = match uuid::Uuid::parse_str(&target_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
    ...
    engine.register_fn("get_combat_targets", move |char_name: String| -> rhai::Array {
        if let Ok(Some(char)) = cloned_db.get_character_data(&char_name) {
            return char.combat.targets.iter().map(|t| {
                let mut map = Map::new();
                map.insert("target_type".into(), Dynamic::from(t.target_type.to_display_string().to_string()));
    ...
    engine.register_fn("get_primary_target_info", move |char_name: String| -> Dynamic {
        ...
        if let Some(target) = char.combat.targets.first() {
            let mut map = Map::new();
            map.insert("target_type".into(), Dynamic::from(target.target_type.to_display_string().to_string()));
    ...
    let cloned_db = db.clone();
    engine.register_fn("get_combat_distance", move |char_name: String, target_id: String| -> String {
        let tid = match uuid::Uuid::parse_str(&target_id) {
            Ok(u) => u,
        ...
        if let Ok(Some(char)) = cloned_db.get_character_data(&char_name) {
            return char.combat.distances.get(&tid)
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_combat_distance", move |char_name: String, target_id: String, distance: String| -> bool {
        let tid = match uuid::Uuid::parse_str(&target_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
    ...
    engine.register_fn("get_distance_modifier", move |weapon_skill: String, distance: String| -> i64 {
    ...
    engine.register_fn("attempt_advance", move |char_name: String, target_id: String| -> bool {
        ...
        if let Ok(Some(mut char)) = cloned_db.get_character_data(&char_name) {
            let current = char.combat.distances.get(&tid).copied().unwrap_or(CombatDistance::Melee);
    ...
    let cloned_db = db.clone();
    engine.register_fn("attempt_retreat", move |char_name: String, target_id: String| -> String {
        let tid = match uuid::Uuid::parse_str(&target_id) {
            Ok(u) => u,
    ...
    let cloned_db = db.clone();
    engine.register_fn("find_compatible_ammo", move |char_name: String, caliber: String| -> Map {
        let mut result = Map::new();
    ...
    let cloned_db = db.clone();
    engine.register_fn("enter_mobile_combat", move |mobile_id: String, target_type: String, target_id: String| -> bool {
        tracing::debug!("enter_mobile_combat called: mobile_id={}, target_type={}, target_id={}", mobile_id, target_type, target_id);
        ...
        if let Ok(Some(mut mobile)) = cloned_db.get_mobile_data(&mid) {
            tracing::debug!("enter_mobile_combat: loaded mobile {} ({}), current in_combat={}, targets={}",
                           mobile.name, mid, mobile.combat.in_combat, mobile.combat.targets.len());

            // For players, just check if we already have a player target
            let already_targeting = if tt == CombatTargetType::Player {
                mobile.combat.targets.iter().any(|t| t.target_type == CombatTargetType::Player)
            } else {
            ...
            if !already_targeting {
                ...
                tracing::debug!("enter_mobile_combat: added target, now {} targets", mobile.combat.targets.len());
            }
            ...
            if save_result.is_ok() {
                if let Ok(Some(verify_mobile)) = cloned_db.get_mobile_data(&mid) {
                    tracing::debug!("enter_mobile_combat: VERIFY after save - {} in_combat={}, targets={}",
                                   mobile_name, verify_mobile.combat.in_combat, verify_mobile.combat.targets.len());
                } else {
    ...
    engine.register_fn("get_mobile_combat_targets", move |mobile_id: String| -> rhai::Array {
        ...
        if let Ok(Some(mobile)) = cloned_db.get_mobile_data(&mid) {
            return mobile.combat.targets.iter().map(|t| {
                let mut map = Map::new();
                map.insert("target_type".into(), Dynamic::from(t.target_type.to_display_string().to_string()));
                map.insert("target_id".into(), Dynamic::from(t.target_id.to_string()));
                Dynamic::from(map)
    ...
    let cloned_db = db.clone();
    engine.register_fn("get_mobile_combat_distance", move |mobile_id: String, target_id: String| -> String {
        let mid = match uuid::Uuid::parse_str(&mobile_id) {
            Ok(u) => u,
            Err(_) => return "melee".to_string(),
        };
        ...
        if let Ok(Some(mobile)) = cloned_db.get_mobile_data(&mid) {
            return mobile.combat.distances.get(&tid)
                .unwrap_or(&CombatDistance::Melee)
                .to_display_string()
                .to_string();
        }
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_mobile_combat_distance", move |mobile_id: String, target_id: String, distance: String| -> bool {
        let mid = match uuid::Uuid::parse_str(&mobile_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
    ...
    let cloned_db = db.clone();
    engine.register_fn("mobile_can_advance", move |mobile_id: String, target_id: String| -> bool {
        let mid = match uuid::Uuid::parse_str(&mobile_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
    ...
    let cloned_db = db.clone();
    engine.register_fn("mobile_attempt_advance", move |mobile_id: String, target_id: String| -> bool {
        let mid = match uuid::Uuid::parse_str(&mobile_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
        ...
        if let Ok(Some(mut mobile)) = cloned_db.get_mobile_data(&mid) {
            let current = mobile.combat.distances.get(&tid).copied().unwrap_or(CombatDistance::Melee);
            if let Some(closer) = current.closer() {
    ...
    engine.register_fn("get_all_characters_in_combat", move || -> rhai::Array {
        cloned_db.get_all_characters_in_combat()
            .unwrap_or_default()
            .into_iter()
    ...
    engine.register_fn("get_all_mobiles_in_combat", move || -> rhai::Array {
        cloned_db.get_all_mobiles_in_combat()
            .unwrap_or_default()
            .into_iter()
    ...
    engine.register_fn("get_leg_wound_penalty", move |char_name: String| -> i64 {
        if let Ok(Some(char)) = cloned_db.get_character_data(&char_name) {
            let left_penalty = char.wounds.iter()
                .filter(|w| w.body_part == BodyPart::LeftLeg)
                .map(|w| w.level.penalty())
                .max()
                .unwrap_or(0);

            let right_penalty = char.wounds.iter()
    ...
    let cloned_db = db.clone();
    engine.register_fn("calculate_flee_chance", move |char_name: String, target_id: String| -> i64 {
        let char = match cloned_db.get_character_data(&char_name) {
            Ok(Some(c)) => c,
    ...
    let cloned_db = db.clone();
    engine.register_fn("remove_mobile_combat_target", move |mobile_id: String, target_name: String| -> bool {
        let mid = match uuid::Uuid::parse_str(&mobile_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_unconscious", move |char_name: String, is_unconscious: bool| -> bool {
        if let Ok(Some(mut char)) = cloned_db.get_character_data(&char_name) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_mobile_unconscious", move |mobile_id: String, is_unconscious: bool| -> bool {
        let mid = match Uuid::parse_str(&mobile_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_mobile_bleedout_rounds", move |mobile_id: String, rounds: i64| -> bool {
        let mid = match Uuid::parse_str(&mobile_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
    ...
    let cloned_db = db.clone();
    engine.register_fn("create_corpse", move |name: String, room_id: String, is_player: bool| -> String {
    ...
    let cloned_db = db.clone();
    engine.register_fn("transfer_inventory_to_corpse", move |char_name: String, corpse_id: String| -> bool {
    ...
    let cloned_db = db.clone();
    engine.register_fn("transfer_equipment_to_corpse", move |char_name: String, corpse_id: String| -> bool {
        let corpse_uuid = match Uuid::parse_str(&corpse_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
    ...
    let cloned_db = db.clone();
    engine.register_fn("transfer_mobile_items_to_corpse", move |mobile_id: String, corpse_id: String| -> bool {
    ...
    let cloned_db = db.clone();
    engine.register_fn("transfer_gold_to_corpse", move |char_name: String, corpse_id: String| -> bool {
        let corpse_uuid = match Uuid::parse_str(&corpse_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_character_spawn_room", move |char_name: String, room_id: String| -> bool {
        if let Ok(Some(mut char)) = cloned_db.get_character_data(&char_name) {
            if room_id.is_empty() {
    ...
    let cloned_db = db.clone();
    engine.register_fn("apply_ongoing_effect", move |char_name: String, effect_type: String, rounds: i64, damage: i64, body_part: String| -> bool {
        if let Ok(Some(mut char)) = cloned_db.get_character_data(&char_name) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("apply_mobile_ongoing_effect", move |mobile_id: String, effect_type: String, rounds: i64, damage: i64, body_part: String| -> bool {
        let mid = match Uuid::parse_str(&mobile_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
    ...
    let cloned_db = db.clone();
    engine.register_fn("clear_ongoing_effects_for_part", move |char_name: String, body_part: String| -> bool {
        if let Ok(Some(mut char)) = cloned_db.get_character_data(&char_name) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("clear_mobile_ongoing_effects_for_part", move |mobile_id: String, body_part: String| -> bool {
        let mid = match Uuid::parse_str(&mobile_id) {
            Ok(u) => u,
            Err(_) => return false,
        };
    ...
    engine.register_fn("get_effective_max_hp", move |char_name: String| -> i64 {
        match cloned_db.get_character_data(&char_name.to_lowercase()) {
            Ok(Some(character)) => {
                let torso_penalty = character.wounds.iter()
    ...
    engine.register_fn("is_arm_disabled", move |char_name: String, side: String| -> bool {
        match cloned_db.get_character_data(&char_name.to_lowercase()) {
            Ok(Some(character)) => {
                ...
                character.wounds.iter().any(|w|
                    (w.body_part == arm_bp || w.body_part == hand_bp)
    ...
    engine.register_fn("get_eye_wound_penalty", move |char_name: String| -> i64 {
        match cloned_db.get_character_data(&char_name.to_lowercase()) {
            Ok(Some(character)) => {
                let left = character.wounds.iter()
                    .filter(|w| w.body_part == BodyPart::LeftEye)
                    .map(|w| w.level.penalty())
                    .max()
                    .unwrap_or(0);
                let right = character.wounds.iter()
...
fn wound_to_map(wound: &Wound) -> Dynamic {
    let mut map = Map::new();
    map.insert("body_part".into(), Dynamic::from(wound.body_part.to_display_string().to_string()));
    map.insert("level".into(), Dynamic::from(wound.level.to_display_string().to_string()));
    map.insert("wound_type".into(), Dynamic::from(wound.wound_type.to_display_string().to_string()));
    map.insert("bleeding_severity".into(), Dynamic::from(wound.bleeding_severity as i64));
