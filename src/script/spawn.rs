...
pub fn register(engine: &mut Engine, db: Arc<Db>) {
    ...
    engine.register_type_with_name::<SpawnPointData>("SpawnPointData")
        ...
        .register_get("entity_type", |sp: &mut SpawnPointData| {
        ...
        .register_get("respawn_interval_secs", |sp: &mut SpawnPointData| sp.respawn_interval_secs)
        ...
        .register_get("spawned_count", |sp: &mut SpawnPointData| sp.spawned_entities.len() as i64)
        .register_get("dependency_count", |sp: &mut SpawnPointData| sp.dependencies.len() as i64);

    // Register SpawnDependency type with getters
    engine.register_type_with_name::<SpawnDependency>("SpawnDependency")
        ...
        .register_get("destination", |dep: &mut SpawnDependency| {
            match &dep.destination {
    ...
    engine.register_fn("create_spawn_point", move |area_id: String, room_id: String, entity_type: String, vnum: String, max_count: i64, interval_secs: i64| {
    ...
    engine.register_fn("list_spawn_points_in_area", move |area_id: String| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&area_id) {
            cloned_db.get_spawn_points_for_area(&uuid)
                .unwrap_or_default()
                .into_iter()
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_spawn_point_enabled", move |spawn_point_id: String, enabled: bool| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&spawn_point_id) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_spawn_point_max_count", move |spawn_point_id: String, max_count: i64| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&spawn_point_id) {
            if let Ok(Some(mut sp)) = cloned_db.get_spawn_point(&uuid) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("set_spawn_point_interval", move |spawn_point_id: String, interval_secs: i64| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&spawn_point_id) {
            if let Ok(Some(mut sp)) = cloned_db.get_spawn_point(&uuid) {
            ...
            for sp in spawn_points {
                ...
                let existing_in_room = match sp.entity_type {
                    SpawnEntityType::Mobile => {
                        cloned_db.get_mobiles_in_room(&sp.room_id)
                            .unwrap_or_default()
                            .iter()
                    ...
                    SpawnEntityType::Item => {
                        cloned_db.get_items_in_room(&sp.room_id)
                            .unwrap_or_default()
                            .iter()
                    ...
                    && (existing_in_room + local_spawned) < sp.max_count
                {
                    let spawned_id = match sp.entity_type {
                        SpawnEntityType::Mobile => {
                            cloned_db.spawn_mobile_from_prototype(&sp.vnum)
                                .ok()
                                .flatten()
                                .and_then(|m| {
                        ...
                        SpawnEntityType::Item => {
                            cloned_db.spawn_item_from_prototype(&sp.vnum)
                                .ok()
                                .flatten()
                                .and_then(|i| {
    ...
    engine.register_fn("get_spawn_dependencies", move |spawn_id: String| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&spawn_id) {
            if let Ok(Some(sp)) = cloned_db.get_spawn_point(&uuid) {
                return sp.dependencies.into_iter()
                    .map(rhai::Dynamic::from)
                    .collect::<Vec<_>>();
            }
        }
    ...
    let cloned_db = db.clone();
    engine.register_fn("add_spawn_dependency", move |spawn_id: String, item_vnum: String, dest_type: String, wear_loc: String, count: i64| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&spawn_id) {
            if let Ok(Some(mut sp)) = cloned_db.get_spawn_point(&uuid) {
    ...
    let cloned_db = db.clone();
    engine.register_fn("add_spawn_dependency", move |spawn_id: String, item_vnum: String, dest_type: String, wear_loc: String, count: i64, chance: i64| {
        if let Ok(uuid) = uuid::Uuid::parse_str(&spawn_id) {
            if let Ok(Some(mut sp)) = cloned_db.get_spawn_point(&uuid) {
                let destination = match dest_type.to_lowercase().as_str() {
                    "inventory" | "inv" => SpawnDestination::Inventory,
