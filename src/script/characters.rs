...
use crate::{
    ...
    fire_environmental_triggers_impl, ActiveBuff, BodyPart, EffectType, WeaponSkill,
};
use std::sync::Arc;
...
pub fn register(engine: &mut Engine, db: Arc<Db>, connections: SharedConnections, state: SharedState) {
    ...
    engine.register_fn("get_class_info", move |class_id: String| -> rhai::Map {
        ...
        if let Some(class) = world.class_definitions.get(&class_id) {
            ...
            let skills_map: rhai::Map = class.starting_skills.iter()
            ...
            let bonuses_map: rhai::Map = class.stat_bonuses.iter()
                .map(|(k, v)| (k.clone().into(), rhai::Dynamic::from(*v as i64)))
                .collect();
    ...
    engine.register_fn("get_random_class", move || -> String {
        let world = state_clone.lock().unwrap();
        let available: Vec<&String> = world.class_definitions.iter()
    ...
    engine.register_fn("get_random_race", move || -> String {
        let world = state_clone.lock().unwrap();
        let available: Vec<&String> = world.race_definitions.iter()
    ...
    engine.register_fn("get_race_info", move |race_id: String| -> rhai::Map {
        ...
        if let Some(race) = world.race_definitions.get(&race_id.to_lowercase()) {
            ...
            // stat_modifiers
            let stat_map: rhai::Map = race.stat_modifiers.iter()
                .map(|(k, v)| (k.clone().into(), rhai::Dynamic::from(*v as i64)))
                .collect();
            map.insert("stat_modifiers".into(), rhai::Dynamic::from(stat_map));
            // granted_traits
            let traits: rhai::Array = race.granted_traits.iter()
                .map(|s| rhai::Dynamic::from(s.clone()))
                .collect();
            map.insert("granted_traits".into(), rhai::Dynamic::from(traits));
            // resistances
            let resist_map: rhai::Map = race.resistances.iter()
                .map(|(k, v)| (k.clone().into(), rhai::Dynamic::from(*v as i64)))
                .collect();
            map.insert("resistances".into(), rhai::Dynamic::from(resist_map));
            // passive_abilities
            let passives: rhai::Array = race.passive_abilities.iter()
                .map(|p| {
                    ...
                    let eff: rhai::Map = p.effects.iter()
                        .map(|(k, v)| (k.clone().into(), rhai::Dynamic::from(*v as i64)))
                        .collect();
            ...
            // active_abilities
            let actives: rhai::Array = race.active_abilities.iter()
                .map(|a| {
    ...
    engine.register_fn("get_racial_resistance", move |char_name: String, damage_type: String| -> i64 {
    ...
    let state_clone = state.clone();
    let cloned_db2 = db.clone();
    engine.register_fn("has_racial_passive", move |char_name: String, passive_id: String| -> bool {
        let race_id = match cloned_db2.get_character_data(&char_name.to_lowercase()) {
            Ok(Some(c)) => c.race.to_lowercase(),
            _ => return false,
        };
    ...
    let state_clone = state.clone();
    let cloned_db2 = db.clone();
    engine.register_fn("get_racial_passive_effect", move |char_name: String, effect_key: String| -> i64 {
        let race_id = match cloned_db2.get_character_data(&char_name.to_lowercase()) {
            Ok(Some(c)) => c.race.to_lowercase(),
            _ => return 0,
        };
    ...
    let cloned_db = db.clone();
    let conns = connections.clone();
    engine.register_fn("toggle_own_builder_flag", move |connection_id: String, is_builder: bool| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&connection_id) {
            let char_name = {
                let conns_guard = conns.lock().unwrap();
                conns_guard.get(&uuid)
    ...
    let cloned_db = db.clone();
    let conns = connections.clone();
    engine.register_fn("set_builder_flag", move |connection_id: String, character_name: String, is_builder: bool| {
        // Verify caller is admin
        if let Ok(caller_uuid) = uuid::Uuid::parse_str(&connection_id) {
            let conns_guard = conns.lock().unwrap();
            let is_admin = conns_guard.get(&caller_uuid)
                .and_then(|s| s.character.as_ref())
                .map(|c| c.is_admin)
                .unwrap_or(false);
            drop(conns_guard);
    ...
    let cloned_db = db.clone();
    let conns = connections.clone();
    engine.register_fn("set_admin_flag", move |connection_id: String, character_name: String, is_admin: bool| {
        // Verify caller is admin
        if let Ok(caller_uuid) = uuid::Uuid::parse_str(&connection_id) {
            let conns_guard = conns.lock().unwrap();
            let caller_is_admin = conns_guard.get(&caller_uuid)
                .and_then(|s| s.character.as_ref())
                .map(|c| c.is_admin)
                .unwrap_or(false);
            drop(conns_guard);
        ...
        if let Ok(uuid) = uuid::Uuid::parse_str(&connection_id) {
            ...
            if let Some(session) = conns_guard.get(&uuid) {
                if let Some(ref char) = session.character {
                    ...
                    let pct = if char.max_hunger > 0 { (char.hunger * 100) / char.max_hunger } else { 0 };
                    map.insert("percent".into(), rhai::Dynamic::from(pct as i64));
                    return rhai::Dynamic::from(map);
                }
            }
    ...
    let conns = connections.clone();
    let cloned_db = db.clone();
    engine.register_fn("get_vision_penalty", move |room_id: String, connection_id: String| -> i64 {
        // Get room
        let room_uuid = match uuid::Uuid::parse_str(&room_id) {
            Ok(u) => u,
            Err(_) => return 0,
        ...
        // Check for light source
        let has_light = cloned_db.get_equipped_items(&char.name)
        ...
        let lighting_penalty: i64 = match lighting {
            "bright" => {
                if has_light_sensitive {
                    let has_glare_protection = cloned_db.get_equipped_items(&char.name)
            ...
            "normal" => 0,
            "dim" => {
                if has_night_vision || has_light || has_dark_adapted { 0 }
                else if has_night_blind { 50 }
                else { 0 }
            }
            "dark" => {
                if has_night_vision || has_light { 0 }
                else if has_night_blind { 50 }
                else if has_dark_adapted { 20 }
                else { 50 }
            }
            "pitch" => {
                if has_night_vision || has_light { 0 }
                else if has_night_blind { 50 }
                else if has_dark_adapted { 40 }
                else { 50 }
            }
            _ => 0,
        };
    ...
    engine.register_fn("add_skill_experience", move |char_name: String, skill_name: String, amount: i64| -> bool {
        match cloned_db.get_character_data(&char_name.to_lowercase()) {
            Ok(Some(mut char)) => {
                ...
                if has_prodigy { xp = xp * 150 / 100; }           // +50%
                else if has_quick_study { xp = xp * 125 / 100; }  // +25%
                if has_slow_learner { xp = xp * 65 / 100; }       // -35%
                xp = xp.max(1);
                entry.experience += xp;
    ...
    let cloned_db = db.clone();
    let state_clone = state.clone();
    engine.register_fn("get_trait_effect_sum", move |char_name: String, effect_key: String| -> i64 {
    ...
    let conns = connections.clone();
    let cloned_db = db.clone();
    engine.register_fn("change_password", move |connection_id: String, new_password: String| -> bool {
        if let Ok(uuid) = uuid::Uuid::parse_str(&connection_id) {
            let mut conns_guard = conns.lock().unwrap();
            if let Some(session) = conns_guard.get_mut(&uuid) {
    ...
    let cloned_db = db.clone();
    let conns = connections.clone();
    engine.register_fn("set_first_character_privileges", move |character_name: String| -> bool {
    ...
    engine.register_fn("get_effective_level", move |char_name: String| -> i64 {
        match cloned_db.get_character_data(&char_name.to_lowercase()) {
            Ok(Some(char)) => {
                char.skills.values()
                    .map(|progress| progress.level as i64)
                    .sum()
            }
            _ => 0,
        }
    ...
    engine.register_fn("get_effective_combat_level", move |char_name: String| -> i64 {
        match cloned_db.get_character_data(&char_name.to_lowercase()) {
            Ok(Some(char)) => {
                let weapon_max = char.skills.iter()
                    .filter_map(|(key, progress)| {
                        WeaponSkill::from_str(key).map(|_| progress.level as i64)
                    })
                    .max()
                    .unwrap_or(0);
    ...
    let cloned_db = db.clone();
    let conns = connections.clone();
    engine.register_fn("apply_buff", move |char_name: String, effect_type_str: String, magnitude: i64, duration_secs: i64, source: String| -> bool {
    ...
    let cloned_db = db.clone();
    let conns = connections.clone();
    engine.register_fn("remove_buff", move |char_name: String, effect_type_str: String| -> bool {
        let effect_type = match EffectType::from_str(&effect_type_str) {
            Some(et) => et,
            None => return false,
        };
    ...
    let cloned_db = db.clone();
    engine.register_fn("get_buff_magnitude", move |char_name: String, effect_type_str: String| -> i64 {
        let effect_type = match EffectType::from_str(&effect_type_str) {
            Some(et) => et,
            None => return 0,
        };
    ...
    engine.register_fn("get_active_buffs", move |char_name: String| -> rhai::Array {
        match cloned_db.get_character_data(&char_name.to_lowercase()) {
            Ok(Some(character)) => {
                character.active_buffs.iter().map(|b| {
                    let mut map = rhai::Map::new();
                    map.insert("effect_type".into(), rhai::Dynamic::from(b.effect_type.to_display_string().to_string()));
    ...
    let cloned_db = db.clone();
    let state_clone = state.clone();
    engine.register_fn("get_effective_stat", move |char_name: String, stat_name: String| -> i64 {
        match cloned_db.get_character_data(&char_name.to_lowercase()) {
            Ok(Some(character)) => {
                ...
                let buff_bonus: i32 = character.active_buffs.iter()
                ...
                let racial_bonus = {
                    ...
                    if let Some(race) = world.race_definitions.get(&race_id) {
                        ...
                        let all_stats: i32 = race.passive_abilities.iter()
                ...
                if stat_key == "intelligence" || stat_key == "int" {
                    let head_penalty = character.wounds.iter()
        ...
        if let Ok(uuid) = uuid::Uuid::parse_str(&connection_id) {
            ...
            if let Some(session) = conns_guard.get(&uuid) {
                if let Some(ref char) = session.character {
                    ...
                    let pct = if char.max_mana > 0 { (char.mana * 100) / char.max_mana } else { 0 };
                    map.insert("percent".into(), rhai::Dynamic::from(pct as i64));
                    return rhai::Dynamic::from(map);
                }
            }
