...
use crate::db::Db;
use crate::types::{
    CatchEntry, DoorState, ExtraDesc, RoomData, RoomExits, RoomFlags, RoomTrigger,
    TriggerType, WaterType,
};
...
fn seed_oakvale(db: &Db) -> Result<usize> {
    ...
    // --- Tavern ---
    let mut tavern = room(seed_uuid("oakvale:tavern"), area, "oakvale:tavern",
        "The Rusty Tankard",
    ...
    // --- Tavern Upstairs ---
    let mut tavern_up = room(seed_uuid("oakvale:tavern_upstairs"), area, "oakvale:tavern_upstairs",
        "Tavern Upper Floor",
    ...
    // --- General Store ---
    let mut store = room(seed_uuid("oakvale:general_store"), area, "oakvale:general_store",
        "General Store",
    ...
    // --- Smithy ---
    let mut smithy = room(seed_uuid("oakvale:smithy"), area, "oakvale:smithy",
        "The Iron Anvil",
    ...
    // --- Temple ---
    let mut temple = room(seed_uuid("oakvale:temple"), area, "oakvale:temple",
        "Temple of Light",
    ...
    // --- Bank ---
    let mut bank = room(seed_uuid("oakvale:bank"), area, "oakvale:bank",
        "Oakvale Bank",
    ...
    // --- Post Office ---
    let mut post = room(seed_uuid("oakvale:post_office"), area, "oakvale:post_office",
        "Post Office",
    ...
    // --- Garden ---
    let mut garden = room(seed_uuid("oakvale:garden"), area, "oakvale:garden",
        "Village Garden",
    ...
    // --- Cottage Lane ---
    let mut cottage_entry = room(seed_uuid("oakvale:cottage_entry"), area, "oakvale:cottage_entry",
        "Cottage Lane",
    ...
    let mut cottage_int = room(seed_uuid("oakvale:cottage_interior"), area, "oakvale:cottage_interior",
        "A Cozy Cottage",
    ...
    // --- North Gate ---
    let mut north_gate = room(seed_uuid("oakvale:north_gate"), area, "oakvale:north_gate",
        "North Gate",
    ...
    // --- East Road ---
    let mut east_road = room(seed_uuid("oakvale:east_road"), area, "oakvale:east_road",
        "East Road",
    ...
    // --- South Path ---
    let mut south_path = room(seed_uuid("oakvale:south_path"), area, "oakvale:south_path",
        "South Path",
    ...
    // --- Market ---
    let mut market = room(seed_uuid("oakvale:market"), area, "oakvale:market",
        "Market Street",
    ...
    let rooms = vec![
        square, tavern, tavern_up, store, smithy, temple, bank, post,
        garden, cottage_entry, cottage_int, north_gate, east_road,
        south_path, market,
    ];
    for r in rooms {
        db.save_room_data(r)?;
...
fn seed_whisperwood(db: &Db) -> Result<usize> {
    ...
    let mut trail_entrance = room(seed_uuid("whisperwood:trail_entrance"), area, "whisperwood:trail_entrance",
    ...
    let mut fork = room(seed_uuid("whisperwood:fork"), area, "whisperwood:fork",
        "Forest Fork",
    ...
    let mut pond = room(seed_uuid("whisperwood:pond"), area, "whisperwood:pond",
        "Tranquil Pond",
    ...
    pond.catch_table = vec![
        CatchEntry { vnum: "oakvale:trout".to_string(), weight: 50, min_skill: 0, rarity: "common".to_string() },
        CatchEntry { vnum: "oakvale:bass".to_string(), weight: 30, min_skill: 1, rarity: "uncommon".to_string() },
    ];
    pond.flags.dirt_floor = true;

    let mut clearing = room(seed_uuid("whisperwood:clearing"), area, "whisperwood:clearing",
        "Sunlit Clearing",
    ...
    let mut herb_garden = room(seed_uuid("whisperwood:herb_garden"), area, "whisperwood:herb_garden",
    ...
    let mut deep_trail = room(seed_uuid("whisperwood:deep_trail"), area, "whisperwood:deep_trail",
        "Deep Forest Trail",
    ...
    let mut wolf_den = room(seed_uuid("whisperwood:wolf_den"), area, "whisperwood:wolf_den",
        "Wolf Den",
    ...
    let mut dark_hollow = room(seed_uuid("whisperwood:dark_hollow"), area, "whisperwood:dark_hollow",
        "Dark Hollow",
    ...
    let mut brook_crossing = room(seed_uuid("whisperwood:brook"), area, "whisperwood:brook",
        "Brook Crossing",
    ...
    let mut grove = room(seed_uuid("whisperwood:grove"), area, "whisperwood:grove",
        "Mossy Grove",
    ...
    let mut forager_camp = room(seed_uuid("whisperwood:forager_camp"), area, "whisperwood:forager_camp",
        "Forager's Camp",
    ...
    let rooms = vec![
        trail_entrance, fork, pond, clearing, herb_garden, deep_trail,
        wolf_den, dark_hollow, brook_crossing, grove, forager_camp,
    ];
    for r in rooms {
        db.save_room_data(r)?;
    }
...
fn seed_ironkeep(db: &Db) -> Result<usize> {
    ...
    let mut approach = room(seed_uuid("ironkeep:approach"), area, "ironkeep:approach",
        "Castle Approach",
    ...
    let mut gatehouse = room(seed_uuid("ironkeep:gatehouse"), area, "ironkeep:gatehouse",
        "Gatehouse",
    ...
    gatehouse.doors.insert("north".to_string(), DoorState {
        name: "iron gate".to_string(),
        is_closed: true,
        is_locked: true,
        key_id: Some(seed_uuid("oakvale:gate_key")),
        description: Some("A heavy iron gate reinforced with thick crossbars. It looks like it requires a key to open.".to_string()),
        keywords: vec!["gate".to_string(), "iron".to_string()],
    });

    let mut courtyard = room(seed_uuid("ironkeep:courtyard"), area, "ironkeep:courtyard",
        "Castle Courtyard",
    ...
    let mut armory = room(seed_uuid("ironkeep:armory"), area, "ironkeep:armory",
        "Armory",
    ...
    let mut great_hall = room(seed_uuid("ironkeep:great_hall"), area, "ironkeep:great_hall",
        "Great Hall",
    ...
    let mut kitchen = room(seed_uuid("ironkeep:kitchen"), area, "ironkeep:kitchen",
        "Castle Kitchen",
    ...
    let mut tower_base = room(seed_uuid("ironkeep:tower_base"), area, "ironkeep:tower_base",
        "Tower Base",
    ...
    let mut elevator = room(seed_uuid("ironkeep:elevator"), area, "ironkeep:elevator",
        "Tower Elevator",
    ...
    let mut tower_mid = room(seed_uuid("ironkeep:tower_mid"), area, "ironkeep:tower_mid",
        "Tower Landing",
    ...
    let mut tower_top = room(seed_uuid("ironkeep:tower_top"), area, "ironkeep:tower_top",
        "Tower Summit",
    ...
    let mut barracks = room(seed_uuid("ironkeep:barracks"), area, "ironkeep:barracks",
        "Barracks",
    ...
    let mut dungeon_stairs = room(seed_uuid("ironkeep:dungeon_stairs"), area, "ironkeep:dungeon_stairs",
        "Dungeon Stairs",
    ...
    let rooms = vec![
        approach, gatehouse, courtyard, armory, great_hall, kitchen,
        tower_base, elevator, tower_mid, tower_top, barracks, dungeon_stairs,
    ];
    for r in rooms {
        db.save_room_data(r)?;
    }
...
fn seed_shadowfang(db: &Db) -> Result<usize> {
    ...
    let mut entrance = room(seed_uuid("shadowfang:entrance"), area, "shadowfang:entrance",
        "Cave Entrance",
    ...
    let mut tunnel = room(seed_uuid("shadowfang:tunnel"), area, "shadowfang:tunnel",
        "Winding Tunnel",
    ...
    let mut goblin_camp = room(seed_uuid("shadowfang:goblin_camp"), area, "shadowfang:goblin_camp",
        "Goblin Camp",
    ...
    let mut fungal_grotto = room(seed_uuid("shadowfang:fungal_grotto"), area, "shadowfang:fungal_grotto",
        "Fungal Grotto",
    ...
    let mut crossroads = room(seed_uuid("shadowfang:crossroads"), area, "shadowfang:crossroads",
        "Cave Crossroads",
    ...
    let mut spider_nest = room(seed_uuid("shadowfang:spider_nest"), area, "shadowfang:spider_nest",
        "Spider Nest",
    ...
    let mut underground_pool = room(seed_uuid("shadowfang:underground_pool"), area, "shadowfang:underground_pool",
        "Underground Pool",
    ...
    underground_pool.catch_table = vec![
        CatchEntry { vnum: "shadowfang:crystal_fish".to_string(), weight: 50, min_skill: 2, rarity: "rare".to_string() },
    ];

    let mut drake_lair = room(seed_uuid("shadowfang:drake_lair"), area, "shadowfang:drake_lair",
        "Shadow Drake's Lair",
    ...
    let mut treasure = room(seed_uuid("shadowfang:treasure_alcove"), area, "shadowfang:treasure_alcove",
        "Treasure Alcove",
    ...
    treasure.doors.insert("west".to_string(), DoorState {
        name: "iron-bound door".to_string(),
        is_closed: true,
        is_locked: true,
        key_id: Some(seed_uuid("shadowfang:treasure_key")),
        description: Some("A heavy door reinforced with iron bands. A large keyhole suggests it requires a specific key.".to_string()),
        keywords: vec!["door".to_string(), "iron".to_string()],
    });

    let mut upper_cave = room(seed_uuid("shadowfang:upper_cave"), area, "shadowfang:upper_cave",
        "Upper Caves",
    ...
    let rooms = vec![
        entrance, tunnel, goblin_camp, fungal_grotto, crossroads,
        spider_nest, underground_pool, drake_lair, treasure, upper_cave,
    ];
    for r in rooms {
        db.save_room_data(r)?;
    }
...
fn seed_hilltop(db: &Db) -> Result<usize> {
    ...
    let mut path = room(seed_uuid("hilltop:path"), area, "hilltop:path",
        "Hilltop Path",
    ...
    let mut farmyard = room(seed_uuid("hilltop:farmyard"), area, "hilltop:farmyard",
        "Farmyard",
    ...
    let mut farmhouse = room(seed_uuid("hilltop:farmhouse"), area, "hilltop:farmhouse",
        "Farmhouse",
    ...
    let mut workshop = room(seed_uuid("hilltop:workshop"), area, "hilltop:workshop",
        "Farm Workshop",
    ...
    let mut barn = room(seed_uuid("hilltop:barn"), area, "hilltop:barn",
        "Barn",
    ...
    let mut garden_plots = room(seed_uuid("hilltop:garden_plots"), area, "hilltop:garden_plots",
        "Garden Plots",
    ...
    let mut orchard = room(seed_uuid("hilltop:orchard"), area, "hilltop:orchard",
        "Apple Orchard",
    ...
    let mut wheat_field = room(seed_uuid("hilltop:wheat_field"), area, "hilltop:wheat_field",
        "Wheat Field",
    ...
    let rooms = vec![
        path, farmyard, farmhouse, workshop, barn, garden_plots,
        orchard, wheat_field,
    ];
    for r in rooms {
        db.save_room_data(r)?;
